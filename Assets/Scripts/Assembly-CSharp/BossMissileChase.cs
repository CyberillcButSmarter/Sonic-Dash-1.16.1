...
public class BossMissileChase : MonoBehaviour
{
	...
	[SerializeField]
	private Transform m_missileSpawnPoint1;

	[SerializeField]
	private Transform m_missileSpawnPoint2;
	...
	public int MissileLauncher { get; set; }
	...
	private IEnumerator performAttackBehaviour()
	{
		...
		m_movementController.MoveToDestination(parameters, false, true);
		yield return null;
		...
		for (; attackIndex < attackCount; attackIndex++)
		{
			...
			if (m_movementController.SplineDistanceFromSonic != attackSettings.Distance)
			{
				...
				m_movementController.MoveToDestination(parameters, true, false);
			...
			if (attackIndex == 0)
			{
				timeToShot += m_animationController.GetCharacterAnimationLength(m_animationController.CurrentPreFiringLongAnimation);
			}
			...
			SpawnMissile(attackLane);
		}
	}

	private void SpawnMissile(Track.Lane lane)
	{
		SplineTracker tracker = m_movementController.GetTracker();
		Spline spline = tracker.Target.getTrackSegment().GetSpline((int)lane);
		...
		case Track.Lane.Left:
			component.Fire(m_missileSpawnPoint1.position, tracker, spline, m_missileOffsetAboveSpline, m_missileSpeed, m_missileAngularVelocity);
			MissileLauncher = 0;
			return;
		case Track.Lane.Right:
			component.Fire(m_missileSpawnPoint2.position, tracker, spline, m_missileOffsetAboveSpline, m_missileSpeed, m_missileAngularVelocity);
			MissileLauncher = 1;
			return;
		}
		if (MissileLauncher == 0)
		{
			component.Fire(m_missileSpawnPoint1.position, tracker, spline, m_missileOffsetAboveSpline, m_missileSpeed, m_missileAngularVelocity);
		}
		else
