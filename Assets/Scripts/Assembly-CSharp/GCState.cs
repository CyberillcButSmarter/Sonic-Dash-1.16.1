...
public class GCState : MonoBehaviour
{
	...
	public const Challenges CurrentChallenge = Challenges.gc6;
	...
	{
		if (s_singleton.m_activeChallenges[5] == State.Active)
		{
			return true;
		}
		return false;
	...
	public static void GCButtonPressed(bool internet)
	{
		for (int i = 0; i < s_singleton.m_activeChallenges.Length; i++)
		{
			if (s_singleton.m_activeChallenges[i] == State.Active)
			{
				...
				{
					s_singleton.m_firstDateGCButtonPressed[i] = DCTime.GetCurrentTime().Date;
					s_singleton.m_isGCButtonPressed[i] = true;
				}
				s_singleton.m_lastDateGCButtonPressed[i] = DCTime.GetCurrentTime().Date;
	...
	private void Event_OnGameFinished()
	{
		...
		{
			GC6Progress.ContributeToChallenge();
		}
	}
	...
	private void Event_OnGameDataLoaded(ActiveProperties activeProperties)
	{
		for (int i = 0; i < m_activeChallenges.Length; i++)
		{
			if (!s_singleton.m_StatusConfirmed[i])
			{
				...
				if (ABTesting.Ready)
				{
					ABTesting.GetTestValueAsInt(ABTesting.Tests.BOOSTERS_RunsBeforeUse);
				}
				if (!PropertyStore.ActiveProperties().DoesPropertyExist(propertyName) || (PlayerStats.GetStat(PlayerStats.StatNames.NumberOfSessions_Total) < 2 && PlayerStats.GetStat(PlayerStats.StatNames.NumberOfRuns_Total) < num))
				{
			...
			{
				m_firstDateGCButtonPressed[i] = DCTime.GetCurrentTime().AddYears(-10).Date;
			}
			...
			{
				m_lastDateGCButtonPressed[i] = DCTime.GetCurrentTime().AddYears(-10).Date;
			}
		}
	}
	...
	private void UpdateStatus()
	{
		...
		for (int i = 0; i < s_singleton.m_activeChallenges.Length; i++)
		{
			...
			if (stateProperty != null && LSONProperties.AsString(stateProperty, out stringValue))
			{
				...
				{
					m_datesChallenges[i] = DCTime.GetCurrentTime().AddDays(7.0).Date;
				}
			}
		}
	}
