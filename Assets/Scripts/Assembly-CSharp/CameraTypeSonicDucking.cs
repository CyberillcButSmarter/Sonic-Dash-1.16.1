...
public class CameraTypeSonicDucking : CameraType
{
	...
	private float CalculateLaneLateralOffset(Track.Lane lane)
	{
		...
		return targetLateralOffset * num;
	}
	...
	private void LateUpdate()
	{
		if (!(m_mainCamera == null))
		{
			...
			Vector3 vector3 = Vector3.Cross(sonicTransform.Forwards, normalized);
			float num2 = Vector3.Angle(vector3, sonicTransform.Up) * ((!(Vector3.Dot(sonicTransform.Up, normalized) > 0f)) ? 1f : (-1f));
