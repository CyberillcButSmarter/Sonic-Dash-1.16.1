...
public class RingStreakConstructor
{
	public struct RingPlacement
	{
		...
		public LightweightTransform FloorTransform
		{
			get
			{
				return m_floorTransformLock.CurrentTransform;
			}
		}
		...
		public GameObject Parent
		{
			get
			{
				return m_floorTransformLock.World.gameObject;
			}
		}
	...
	private class RingGroup
	{
		...
		public int SuffixEmptyRowCount
		{
			get
			{
				return m_postGroupEmptyRows;
			}
		}
		...
		public void AddOpenRowToGroup(bool[,] openRow, Track.Lane? missingLane, SplineTracker rowKeyframe)
		{
			...
			Track.Lane? previousMissingLane = ((!m_gapRows.ContainsKey(m_rows.Count - 1)) ? ((Track.Lane?)null) : new Track.Lane?(m_gapRows[m_rows.Count - 1]));
	...
	private class Node
	{
		...
		public uint Substreaks
		{
			get
			{
				return m_substreakMask;
			}
		}
	...
	private IEnumerable<RingPlacement> PlaceRingsOnLanes(IEnumerable<Node> laneNodes, SplineTracker middleSplineTracker, float jumpHeight)
	{
		...
		laneTransforms[0] = GetTransformOnLane(middleSplineTracker, Track.Lane.Left);
		laneTransforms[1] = GetTransformOnLane(middleSplineTracker, Track.Lane.Middle);
		laneTransforms[2] = GetTransformOnLane(middleSplineTracker, Track.Lane.Right);
