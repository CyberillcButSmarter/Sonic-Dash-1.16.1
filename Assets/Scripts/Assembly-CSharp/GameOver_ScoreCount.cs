...
public class GameOver_ScoreCount : GameOver_Component
{
	...
	public static bool HighScoreVisible
	{
		get
		{
			return m_highScoreVisible;
		}
	}
	...
	public override void ProcessFinished()
	{
		...
		{
			m_doubleRingBlocker.gameObject.SetActive(false);
		}
		...
		if (Dialog_VideoRecorderPrompt.CheckIfDisplay())
		{
			Dialog_VideoRecorderPrompt.Display();
			return;
		}
		if (GCState.IsCurrentChallengeActive() && true && !GC6Progress.ChallengeFullycompleted())
		{
			...
			if (!GC6Progress.PageVisitedFromResult)
			{
				m_gcTokenButtonLoopAnimation.enabled = true;
				GC6ButtonAnimationDisplayer.m_keepActive = true;
			}
			...
			else if (GC6Progress.IsRewardDue())
			{
				Dialog_GCPendingReward.Display();
			}
		}
		GoogleSpecialEvent.CheckForSuccessOrFailure();
	}

	public override void ActiveTransitionFinished(MonoBehaviour transitioningObject)
	{
		State state = (State)0;
		if (transitioningObject.name == "Score Panel GC (Show) [Trigger]" || transitioningObject.name == "Score Panel (Show) [Trigger]")
		{
			state = State.DisplayVisible;
		}
		else if (transitioningObject.name == "Ring Count (Show) [Trigger]")
		{
	...
	private bool DisplayInitialise(float timeDelta)
	{
		if (GCState.IsCurrentChallengeActive() && true && !GC6Progress.ChallengeFullycompleted())
		{
			...
			GC4ButtonAnimationDisplayer.m_keepActive = false;
	...
	private bool DisplayTrigger(float timeDelta)
	{
		...
		Audio.PlayClip(m_audioPanelShown, false);
		return false;
	}
	...
	private bool ScoreCountCount(float timeDelta)
	{
		...
		if (m_currentScoreIncrease > 1f)
		{
			...
			Audio.PlayClip(m_audioCountingScore, false);
		}
		if (m_currentScoreDisplay >= (float)ScoreTracker.CurrentScore)
		{
			...
			if (m_gcTokenStampAnimation != null)
			{
				...
				m_GCTokensValue.text = "+ " + GC6Progress.CurrentCollectedThisRun;
			}
		...
		if ((m_state & State.BoosterBonusVisible) != State.BoosterBonusVisible && m_currentBonusScoreDisplay > 0f)
		{
			...
			Audio.PlayClip(m_audioBoosterScoreBonus, false);
			m_pauseTimeCount = 0f;
		}
		if ((m_state & State.HighScoreVisible) != State.HighScoreVisible && m_targetHighScore != 0f && m_currentScoreDisplay > m_targetHighScore)
		{
			...
			Audio.PlayClip(m_audioHighScoreAchieved, false);
	...
	private bool RingShowDouble(float timeDelta)
	{
		...
		Audio.PlayClip(m_audioDoubleRingsShown, false);
		return false;
	}
	...
	private bool RingsCacheContentDisplay(float timeDelta)
	{
		...
		m_buyRingsBlocker.gameObject.SetActive(false);
	...
	private bool RingCount(float timeDelta)
	{
		...
		if (m_currentRingsIncrease >= 1f)
		{
			...
			Audio.PlayClip(m_audioCountingRings, false);
