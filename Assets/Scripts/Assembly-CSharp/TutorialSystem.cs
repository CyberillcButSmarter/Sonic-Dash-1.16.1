...
public class TutorialSystem : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		m_progressCounts = new int[m_maximumNumberOfTrackTutorialDialogs];
		Reset(true);
	}
	...
	private void Event_OnSpringEnd()
	{
		if (m_showTutorial && !m_completePopupShown)
		{
			...
			{
				PropertyStore.Store("Tutorial Star Awarded", true);
		...
		if ((bool)m_popupCompleteTrigger)
		{
			...
			m_popupCompleteTrigger.SendMessage("OnClick", SendMessageOptions.DontRequireReceiver);
		}
			m_trackTutorialShown = true;
	...
	private void Event_OnTarget()
	{
		...
		if ((bool)m_popupChopperTapTrigger)
		{
			...
			m_popupChopperTapTrigger.SendMessage("OnClick", SendMessageOptions.DontRequireReceiver);
		}
	}
	...
	public void NotifyTemplate(string name, float distance)
	{
		...
		if (dialogType != DialogType.Undefined)
		{
	...
	public void UpdateSystem(float distance)
	{
		if (m_dialogToCheck < m_numberOfRegisteredTutorialTriggerPoints && m_dialogTriggers[m_dialogToCheck].m_distance < distance)
		{
			...
			if (flag)
			{
				...
				m_progressCounts[m_dialogToCheck]++;
				GameAnalytics.TutorialProgress(m_progressCounts);
			}
			m_dialogToCheck++;
