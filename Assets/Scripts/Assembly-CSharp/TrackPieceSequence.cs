...
public class TrackPieceSequence
{
	...
	private TrackDatabase.TrackPiecePrefab PickRandomPrefab(IList<TrackDatabase.TrackPiecePrefab> source, TrackDatabase.PieceType typeToPick, ElevationChanges changesAllowed)
	{
		...
		int num;
		switch (changesAllowed)
		{
		case ElevationChanges.ForceToLow:
			num = ((m_previousElevation > 0) ? 3 : 0);
			break;
		case ElevationChanges.FollowSequence:
			num = (int)GetCurrentElevationType();
			break;
		default:
			num = ((m_previousElevation > 0) ? 2 : 0);
			break;
		}
		TrackDatabase.ElevationType elevationType = (TrackDatabase.ElevationType)num;
