...
public class BossMineDeployment : MonoBehaviour
{
	...
	private IEnumerator performMineBehaviour(float timeToGetIntoPosition)
	{
		...
		do
		{
			...
			m_movementController.SetOrientation(Quaternion.Euler(currentEuler) * Vector3.forward, false);
		}
		...
		if (attackCount > 0)
		{
			...
			float preFireAnimDistanceRequired = maxSpeed * m_animationController.GetCharacterAnimationLength(BossAnim.PreFireMine);
