...
{
	public static readonly long FacebookAppID = 301289123310261L;
	...
	public HLSocialPlatform()
	{
		...
		Social.Active = this;
		socialInitialise(FacebookAppID);
	}
	...
	public HLUserProfile GetUserProfileFromUserName(string userName)
	{
		foreach (HLUserProfile value in m_userProfiles.Values)
		{
	...
	public void Authenticate(ILocalUser user, Action<bool, string> callback)
	{
		...
		{
			callback?.Invoke(success, success ? string.Empty : "Authentication failed");
		});
	}
	...
	public void OnFacebookAuthenticateComplete(string Param)
	{
		...
		if (flag)
		{
			...
			if (array.Length > 2)
			{
				m_user.setFacebookAccessToken(array[3]);
				m_user.setFacebookMailAdress(array[4]);
			}
			SLAnalytics.LogPushTagWithParameters("FB Login", "1", "string");
		}
		Debug.Log("GENERATING EVENT OnFacebookAuthenticateComplete");
	...
	public void OnGameCenterAuthenticateComplete(string Param)
	{
		...
		if (flag)
		{
			PlayerStats.instance().OnGooglePlusLogin();
			Community.g_instance.OnAuthenticationResult(true);
		}
	}
	...
	{
		m_user.setFacebookAuthenticated(false);
		SettingsFacebookLogInOutControl.ChangeFacebookConnectionOption(false, true);
	}
	...
	{
		socialInviteFriend(user, LanguageStrings.First.GetString("INVITE_HEADING"), LanguageStrings.First.GetString("INVITE_MESSAGE"));
	}
}
