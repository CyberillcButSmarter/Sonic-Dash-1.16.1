...
public class MotionSprintState : MotionState
{
	...
	public override LightweightTransform CalculateNewTransform(TransformParameters tParams)
	{
		...
		if (physics.IdealSpeed > 0f)
		{
			...
			if ((CharacterManager.Singleton.GetCurrentCharacter() == Characters.Type.Tails || CharacterManager.Singleton.GetCurrentCharacter() == Characters.Type.Shadow || CharacterManager.Singleton.GetCurrentCharacter() == Characters.Type.Rouge) && num > 0.95f)
			{
				num = 0f;
			}
