...
public class LSONReader
{
	...
	private static string[] MaskedSplit(string line, string splitDelimiter, string maskDelimiter)
	{
		bool flag = false;
		int num = 0;
		int num2 = 0;
		...
		char c = splitDelimiter.ToCharArray()[0];
		char c2 = maskDelimiter.ToCharArray()[0];
		char[] array2 = array;
		foreach (char c3 in array2)
		{
			if (c3 == c)
			{
				if (!flag)
				{
					list.Add(line.Substring(num, num2 - num));
					num = num2 + 1;
				}
			}
			else if (c3 == c2)
	...
	private static string[] GetProperty(string line)
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			array[i] = array[i].Trim().Replace("'", string.Empty);
			if (array[i].Length == 0)
			{
				flag = false;
			}
