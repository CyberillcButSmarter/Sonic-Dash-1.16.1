...
public class SonicAudioControl : MonoBehaviour
{
	...
	private void Start()
	{
		...
		EventDispatch.RegisterInterest("SpringTokenHuntSuccess", this);
		m_nextSFXToPlay = 0;
	...
	private void Update()
	{
		if (DashMonitor.instance().isDashing() || HeadstartMonitor.instance().isHeadstarting())
		{
			...
			{
				Audio.PlayClip(m_dashOnceAudioClip, false);
				m_dashActive = Audio.PlayClip(m_dashLoopedAudioClip, true);
			}
		}
		else
		{
	...
	{
		Audio.PlayClip(m_rollSpinUpAudioClip, false);
	}

	public void PlayRollGoSFX()
	{
		Audio.PlayClip(m_rollGoAudioClip, false);
	}
	...
	{
		Audio.PlayClip(m_jumpAudioClip, false);
	}

	public void PlayDeathSFX()
	{
		Audio.PlayClip(m_enemyCollisionClip, false);
	}

	public void PlayRespawnSFX()
	{
		Audio.PlayClip(m_respawnAudioClip, false);
	}

	public void PlayPowerUpMagnetPickupSFX()
	{
		Audio.PlayClip(m_powerUpPickUpMagnetAudioClip, false);
	}

	public void PlayPowerUpShieldPickupSFX()
	{
		Audio.PlayClip(m_powerUpPickUpShieldAudioClip, false);
	}

	public void PlayPowerUpRingsPickupSFX()
	{
		Audio.PlayClip(m_powerUpPickUpRingsAudioClip, false);
	}
	...
	public void PlaySpringSFX()
	{
		...
		{
			Audio.PlayClip(m_boosterSpringAudioClip, false);
		}
		else
		{
			Audio.PlayClip(m_springAudioClip, false);
		}
	}
	...
	private void OnEnterSetPiece()
	{
		...
		{
			Audio.PlayClip(m_dashOnceAudioClip, false);
			Audio.PlayClip(m_setPieceDashPad, false);
	...
	{
		Transform dashStartSource = Audio.PlayClip(m_setPieceDashStart, false);
		while (Audio.IsPlaying(dashStartSource) || Audio.Paused)
		{
			yield return null;
		}
		Audio.PlayClip(m_setPieceDashLoop, true);
	}
	...
	private void Event_OnSingleSpringGestureSuccess(CartesianDir dir)
	{
		...
		{
			Audio.PlayClip(m_springGestureSfx[m_nextSFXToPlay], false);
		}
		m_nextSFXToPlay++;
		if (m_nextSFXToPlay == m_springGestureSfx.Length)
		{
	...
	{
		Audio.PlayClip(m_springGestureSuccess, false);
	}

	private void Event_SpringTokenHuntSuccess()
	{
		Audio.PlayClip(m_springGestureSuccess, false);
	}
	...
	public void OnSlam()
	{
		...
		{
		case MaterialType.Grass:
			Audio.PlayClip(m_diveSlamAudioClipGrass, false);
			break;
		case MaterialType.Stone:
			Audio.PlayClip(m_diveSlamAudioClipStone, false);
			break;
		}
	}
	...
	{
		Audio.PlayClip(m_hopAudioClip, false);
	}

	public void PlayStrafeSFX()
	{
		Audio.PlayClip(m_strafeAudioClip, false);
	}

	public void StartAttackAnim(Pair<float, float> SpinUpTimeTargetVelocity)
	{
		Audio.PlayClip(m_HomingAttackAudioClip, false);
	}

	public void StartStumbleAnim()
	{
		Audio.PlayClip(m_DamageAudioClip, false);
	}
}
