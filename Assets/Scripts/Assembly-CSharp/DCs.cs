...
public class DCs : MonoBehaviour
{
	...
	public static int NumberOfPeices
	{
		get
		{
			return s_singleton.m_piecesCollected.Length;
		}
	}
	...
	{
		s_singleton.m_DCTimeValidation.EnforceValidTime(true);
	...
	public static int GetNumPiecesRemaining()
	{
		return NumberOfPeices - s_singleton.NumPiecesCollected();
	}
	...
	public static string GetTimeRemaining()
	{
		...
		return string.Format("{0}:{1}:{2}", num.ToString("D2"), num2.ToString("D2"), num3.ToString("D2"));
	}
	...
	public static void ChangeDay()
	{
		if (!s_singleton.m_challengeCompleted || ChallengeRewarded)
		{
			if (!s_singleton.m_challengeCompleted || DaySkipped())
			{
				...
				PlayerStats.GetCurrentStats().m_trackedStats[83] = 0;
			...
			s_singleton.ResetPiecesCollected(false);
		}
	}
	...
	public static void SetToCheatingState(bool save)
	{
		...
		s_singleton.ResetPiecesCollected(true);
		if (save)
		{
			PropertyStore.Save();
		}
	...
	{
		return s_singleton.m_spawnChallengePiece && !TutorialSystem.instance().isTrackTutorialEnabled();
	}
	...
	{
		m_DCTimeValidation.EnforceValidTime(true);
		CheckData();
	...
	private void Event_OnGameDataLoaded(ActiveProperties activeProperties)
	{
		...
		m_DCTimeValidation.EnforceValidTime(false);
		CheckData();
	}
}
