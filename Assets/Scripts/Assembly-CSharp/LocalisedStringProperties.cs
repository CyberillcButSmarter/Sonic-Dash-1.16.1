...
public class LocalisedStringProperties : MonoBehaviour
{
	...
	public string GetLocalisedString()
	{
		...
		if (languageStrings == null)
		{
			throw new UnityException(string.Format("LocalisedStringProperties::GetLocalisedString - Unable to find a Language Strings object to get the localised string from in '{0}'", base.name));
		}
	...
	private void StoreGuiText()
	{
		...
		{
			throw new UnityException(string.Format("LocalisedStringProperties::StoreGuiText - Unable to find a UILabel component on '{0}'", base.name));
		}
	}
}
