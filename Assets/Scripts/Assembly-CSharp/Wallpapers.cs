...
public class Wallpapers : MonoBehaviour
{
	...
	private IEnumerator DownloadWallpaperBundles()
	{
		...
		for (int j = 0; j < m_papersToDownload.Count; j++)
		{
			m_papersToAward.Add(m_papersToDownload[j]);
		}
	...
	private IEnumerator UpdatePendingAwards()
	{
		...
		for (int paperGroupIndex = 0; paperGroupIndex < m_papersToAward.Count; paperGroupIndex++)
		{
			...
			{
				wallpaperName = string.Format("{0}-low", wallpaperName);
			}
			AssetBundleRequest request = thisBundle.LoadAssetAsync<Texture2D>(wallpaperName);
			...
			Texture2D wallpaperTexture = request.asset as Texture2D;
				...
				{
					thisDownload.m_loadedBundles[awardIndex].Unload(true);
		...
		for (int i = 0; i < m_papersToAward.Count; i++)
		{
			DownloadingWallpapers thisDownload2 = m_papersToAward[i];
			thisDownload2.m_rootEntry.m_state &= ~StoreContent.StoreEntry.State.Downloading;
			for (int j = 0; j < thisDownload2.m_awardedEntries.Length; j++)
			{
				StoreContent.StoreEntry entryToAward2 = thisDownload2.m_awardedEntries[j];
				if (entryToAward2 != null)
				{
					entryToAward2.m_state &= ~StoreContent.StoreEntry.State.Downloading;
				}
			}
		}
