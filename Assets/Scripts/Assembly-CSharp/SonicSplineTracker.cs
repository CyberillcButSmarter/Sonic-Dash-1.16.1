...
public class SonicSplineTracker : MonoBehaviour
{
	...
	public bool IsRolling
	{
		get
		{
			return m_motionStateMachine != null && m_motionStateMachine.HasActiveState && m_motionStateMachine.CurrentState is MotionRollState;
		}
	}

	public SonicHandling Handling
	{
		get
		{
			return Sonic.Handling;
		}
	}
	...
	public float Speed
	{
		get
		{
			return (!base.enabled || m_internalTracker == null) ? 0f : m_internalTracker.TrackSpeed;
		}
	}
	...
	public Spline CurrentSpline
	{
		get
		{
			return (!base.enabled || m_internalTracker == null) ? null : m_internalTracker.Target;
		}
	}
	...
	public LightweightTransform IdealPosition
	{
		get
		{
			return m_internalTracker.CurrentSplineTransform;
		}
	}
	...
	public SplineTracker InternalTracker
	{
		get
		{
			return m_internalTracker;
		}
	}

	public MotionStateMachine InternalMotionState
	{
		get
		{
			return m_motionStateMachine;
		}
	}

	public float HeightAboveLowGround
	{
		get
		{
			return base.transform.position.y - s_worldTransform.position.y;
		}
	}
	...
	public bool IsTrackerAvailable
	{
		get
		{
			return m_internalTracker != null && m_internalTracker.CanUpdate;
		}
	}
	...
	{
		return (m_ghostedTime > 0f) ? true : false;
	}
...
public void Awake()
{
	...
	{
		Debug.LogWarning("SonicSplineTracker: no Track found in scene; using current transform as fallback.");
		m_gameStartTrackTransform = new LightweightTransform(base.transform);
	}
	...
	private void Event_OnTrackGenerationComplete()
	{
		...
		if (m_toggleOnBelowWater == null)
		{
			...
			{
				m_toggleOnBelowWater.SetActive(false);
			}
		}
		SetToggleObjectsActive();
		TrackerOverGap = false;
	...
	public void Update()
	{
		...
		MotionState.TransformParameters tParams = new MotionState.TransformParameters
		{
			Tracker = m_internalTracker,
			StateMachine = m_motionStateMachine,
			CurrentTransform = lightweightTransform,
			Physics = m_physics,
			Track = m_track,
			OverGap = TrackerOverGap,
			OverSmallIsland = TrackerOverSmallIsland
		};
		...
		if (segmentOfSpline != null)
		{
			...
			{
				Delta = previousDelta,
				PrevTrackPosition = TrackPosition,
				NewTrackPosition = num,
				Lane = Track.GetLaneOfSpline(CurrentSpline)
			};
	...
	public void triggerRespawn(bool usedPowerUp)
	{
		...
		if (usedPowerUp)
		{
			Sonic.AnimationControl.TriggerRespawnPowerupEffect(false);
	...
	private void ClearTrackForRespawn()
	{
		...
		for (int i = 0; i < m_entityScratchList.Count; i++)
		{
			...
			TrackGameObject trackGameObject = trackEntity as TrackGameObject;
			if (trackGameObject == null || !trackGameObject.IsValid)
			{
				continue;
			}
	...
	private void OnEnterSetPiece()
	{
		...
		GetComponentInChildren<DashFX>().ForceIsDashFXActive(true);
	...
	private void OnLeftSetPiece()
	{
		...
		GetComponentInChildren<DashFX>().ForceIsDashFXActive(false);
		m_forceRotationTimer = 1f;
	}
	...
	public void OnTriggerStay(Collider other)
	{
		if (base.enabled && !m_isInvulnerable && m_motionStateMachine.CurrentState.GetType() == typeof(MotionGroundStrafeState))
		{
			...
			{
				bool heldRings = ((RingStorage.HeldRings > 0) ? true : false);
	...
	public void OnTriggerEnter(Collider other)
	{
		...
		if (component6 != null)
		{
			...
			{
				DCs.SetChallengePieceSpawn(true);
			}
			else
			{
	...
	public IEnumerator WaitForCleanTrack()
	{
		...
		m_camera.SetOneTimeOffset(-trackToSonic);
		m_lastCameraTransform.Location = m_camera.transform.position;
	}
	...
	private DeathState getDeathState()
	{
		...
		{
			IsDead = (flag2 || flag3),
			DeathType = endBehaviourInCurrentDirection
		};
	}
