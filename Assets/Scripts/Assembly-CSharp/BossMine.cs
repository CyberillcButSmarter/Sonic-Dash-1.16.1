...
public class BossMine : Hazard
{
	...
	private IEnumerator performBehaviour(Vector3 spawnPoint, Spline spline, float preArmingPause, float preDroppingPause)
	{
		...
		{
			Vector3 position2 = m_tracker.CurrentSplineTransform.Location + desiredArmingOffset;
		...
		while (m_distanceTravelled < droppedDistance)
		{
			...
			Vector3 currentOffset2 = Vector3.Lerp(desiredArmingOffset, desiredDroppedMineOffset, dropProgress);
			Vector3 position3 = m_tracker.CurrentSplineTransform.Location + currentOffset2;
