...
internal class Spikes : Enemy
{
	...
	private IEnumerator Patrol(Direction initialDir)
	{
		...
		switch (initialDir)
		{
		case Direction.ToPlayersRight:
			num = 1;
			break;
		case Direction.ToPlayersLeft:
			num = 3;
			break;
		default:
			num = 0;
		...
		Direction_2D currentDir = (Direction_2D)num;
		int currentWaypointIndex = 0;
		...
		while (true)
		{
			...
			WorldTransformLock currentWorldLock2 = patrolWaypoints[currentWaypointIndex];
			Direction_2D newDir = ((nextWaypointIndex >= currentWaypointIndex) ? Direction_2D.Right : Direction_2D.Left);
			m_animComponent.Stop();
			if (newDir != currentDir)
			{
				string animName2 = GetAnimName(currentDir, newDir);
				m_animComponent[animName2].wrapMode = WrapMode.Once;
				m_animComponent.Play(animName2);
				m_animComponent.Sample();
				while (m_animComponent.isPlaying)
				{
					followTracker(currentWorldLock2);
					yield return null;
				}
				currentDir = newDir;
			}
			...
			while (!BeatGenerator.instance().beatThisFrame() || !m_movementPermitted)
			{
				if (!m_movementPermitted && !m_lockedInPlace)
				{
					string animName3 = m_attackAnim;
					m_animComponent[animName3].wrapMode = WrapMode.Loop;
					m_animComponent.Play(animName3);
					float timer2 = 0.6f;
					while (timer2 > 0f)
					{
						float progress2 = 1f - timer2 / 0.6f;
						progress2 = Mathf.Clamp(progress2, 0f, 1f);
						Quaternion destinationRotation2 = CalculateRotationOnSpline(Direction_2D.Forwards, currentWorldLock2.CurrentTransform.Orientation);
						base.transform.rotation = Quaternion.Lerp(base.transform.rotation, destinationRotation2, progress2);
			...
			yield return StartCoroutine(Patrol_MoveToSpline(toLock: patrolWaypoints[nextWaypointIndex], currentDir: currentDir, fromLock: currentWorldLock2, nextWaypointIndex: nextWaypointIndex));
	...
	private WorldTransformLock[] CalculatePatrolWaypoints(ref int currentWaypointIndex)
	{
		...
		for (int i = 0; i < 3; i++)
		{
			...
			array[i] = new WorldTransformLock(spline, spline.GetTransform(closestPoint.LineDistance));
