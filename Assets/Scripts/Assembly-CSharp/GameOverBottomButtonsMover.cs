...
public class GameOverBottomButtonsMover : MonoBehaviour
{
	...
	{
		bool flag = GrantooManager.IsGrantooAvailable() && !GrantooManager.GetInstance().IsActive();
		m_Buttons[4].SetActive(flag);
		m_PlayLabel.text = LanguageStrings.First.GetString((!flag) ? "BUTTON_PLAY" : "BUTTON_SINGLE");
		int num = 0;
		int num2 = 0;
