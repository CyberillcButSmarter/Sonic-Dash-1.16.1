...
public class Dialog_CloudConflict : MonoBehaviour
{
	...
	private void Populate()
	{
		...
		{
			m_remoteRingsLabel.text = CloudStorage.GetCloudProperty("Banked Rings Total", false);
		}
		if (m_remoteRSRsLabel != null)
		{
			m_remoteRSRsLabel.text = CloudStorage.GetCloudProperty("Star Rings Total", false);
		}
		if (m_remoteHighScoreLabel != null)
		{
			m_remoteHighScoreLabel.text = CloudStorage.GetCloudProperty(ScoreTracker.HighestScoreSavedProperty, false);
		}
		if (m_remoteDescLabel != null)
		{
			m_remoteDescLabel.text = CloudStorage.GetCloudPropertyDeviceDesc(false);
		}
	}

	private void Trigger_ConflictKeepLocal()
	{
		CloudStorage.Save(true);
	}
