...
public class GameState : MonoBehaviour
{
	...
	private const string BossRoot = "bosses";

	private const string LostWorldEventProperty = "lostworldevent";

	private const string ZazzBossProperty = "zazzboss";

	private const string EggmanBossProperty = "eggmanboss";

	private const float LeaderboardTimeOut = 0f;

	private const int DefaultNumLeaderboardEntries = 30;
	...
	public static float TimeInGame
	{
		get
		{
			return (!(g_gameState == null)) ? (Time.time - g_gameState.m_gameStartTime) : Time.timeSinceLevelLoad;
		}
	}
	...
	public static bool LostWorldEventActive { get; private set; }
	...
	private void UpdateForServer()
	{
		...
		BossLoader.ZazzBossActive = false;
		BossLoader.EggmanBossActive = true;
		if (FeatureState.Valid)
		{
			...
			stateProperty = FeatureState.GetStateProperty("bosses", "zazzboss");
			if (stateProperty != null)
			{
				bool boolValue2 = false;
				if (LSONProperties.AsBool(stateProperty, out boolValue2))
				{
					BossLoader.ZazzBossActive = boolValue2;
				}
			}
			stateProperty = FeatureState.GetStateProperty("bosses", "eggmanboss");
			if (stateProperty != null)
			{
				bool boolValue3 = false;
				if (LSONProperties.AsBool(stateProperty, out boolValue3))
				{
					BossLoader.EggmanBossActive = boolValue3;
				}
			}
		}
		BossLoader.ConfirmNextBoss();
	}
	...
	{
		CacheLeaderboards(30);
	}

	public void CacheLeaderboards(int numEntries)
	{
