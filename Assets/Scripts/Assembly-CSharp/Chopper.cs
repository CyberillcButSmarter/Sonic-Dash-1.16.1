...
internal class Chopper : Enemy
{
	...
	{
		m_chopperGO.SetActive(false);
	}

	private void makeVisible()
	{
		m_chopperGO.SetActive(true);
	}
	...
	private IEnumerator Patrol()
	{
		...
		while (!finished)
		{
			...
			m_animComponent.Stop();
			yield return null;
			m_animComponent[m_swimAnimName].wrapMode = WrapMode.Loop;
			m_animComponent.Play(m_swimAnimName);
			m_animating = true;
			while (true)
			{
				float sonicTrackPosition2 = Sonic.Tracker.TrackPosition;
				if (base.TrackDistance - sonicTrackPosition2 < -10f && m_activeInTargetManager)
				{
					TargetManager.instance().NotifyChopperInactive();
					m_activeInTargetManager = false;
				}
