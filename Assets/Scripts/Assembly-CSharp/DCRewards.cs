...
public class DCRewards : MonoBehaviour
{
	...
	public static StoreContent.StoreEntry GetFinalDayReward(int index, out int quantity, bool getFinalQuantity)
	{
		...
		if (index >= 4)
		{
			storeEntry = s_rewards.GetLastRewardOnFinalDay(out quantity, getFinalQuantity);
		}
		else
		{
	...
	public static void ReplaceReward(string rewardToReplace, string newReward)
	{
		for (int i = 0; i < s_rewards.m_dailyRewardIDs.Length; i++)
		{
			if (string.Compare(s_rewards.m_dailyRewardIDs[i], rewardToReplace, StringComparison.InvariantCultureIgnoreCase) == 0)
			{
				s_rewards.m_dailyRewardIDs[i] = newReward;
			}
		}
		for (int j = 0; j < s_rewards.m_finalRewardID.Length; j++)
		{
			if (string.Compare(s_rewards.m_finalRewardID[j], rewardToReplace, StringComparison.InvariantCultureIgnoreCase) == 0)
			{
				s_rewards.m_finalRewardID[j] = newReward;
			}
		}
	}
	...
	private StoreContent.StoreEntry GetLastRewardOnFinalDay(out int quantity, bool getFinalQuantity)
	{
		StoreContent.StoreEntry storeEntry = null;
		...
		for (int i = 0; i < m_characterOrder.Length; i++)
		{
			...
			if (type2 != Characters.Type.Sonic && !Characters.CharacterUnlocked(type2))
			{
				type = type2;
				break;
			}
		}
		if (type != Characters.Type.Sonic)
		{
		...
		else
		{
			...
			{
				quantity = m_finalRewardQuantity[4];
			}
		}
		return storeEntry;
	}
