...
public class BossAttack : MonoBehaviour
{
	...
	private Transform m_vehicleCenterTransform;
	...
	private void OnEnable()
	{
		...
		if (BossLoader.Instance().m_currentBoss == BossLoader.Bosses.Eggman)
		{
			m_vehicleCenterTransform = GameObject.Find("ship_bone").transform;
		}
		else if (BossLoader.Instance().m_currentBoss == BossLoader.Bosses.Zazz)
		{
			m_vehicleCenterTransform = GameObject.Find("moon_bone_body").transform;
		}
	}
	...
	private void Update()
	{
		...
		Sonic.ScoreAnchor.transform.position = m_vehicleCenterTransform.position;
	...
	private IEnumerator WaitForBossArrival()
	{
		...
		m_movementController.MoveToDestination(moveParams, m_enableBossDrift, true);
		yield return new WaitForEndOfFrame();
		...
		{
			float progress = Mathf.Clamp01(m_attackProgressCurve.Evaluate(timer));
			Vector3 position = Vector3.Lerp(m_attackStartInSonicLocalSpace, m_attackEndInSonicLocalSpace, progress);
			position = Sonic.Transform.rotation * position;
			position += Sonic.Transform.position;
			m_movementController.MoveToDestination(position, m_enableBossDrift, true);
	...
	private IEnumerator PlayOutcomeFail()
	{
		...
		Boss.GetInstance().AudioController.PlayQTEEscape1SFX();
	...
	private BossAnim GetFailAnim()
	{
		...
		{
		case 0:
			return BossAnim.AttackFailure1;
		case 1:
			return BossAnim.AttackFailure2;
		case 2:
			return BossAnim.AttackFailure3;
		default:
			return BossAnim.Idle;
		}
	}
	...
	private void Event_OnBossBattleOutroEnd()
	{
		if (!m_attackSuccess)
		{
			StartCoroutine("PlayBattleOutroEnd");
		}
	}

	private IEnumerator PlayBattleOutroEnd()
	{
		yield return new WaitForSeconds(0.5f);
		Boss.GetInstance().AudioController.PlayQTEEscape2SFX();
	...
	private void EndBossAttackGesture()
	{
		...
		case 0:
			...
			Sonic.ScoreAnchor.SetActive(true);
		...
		case 2:
			...
			m_visualController.Flash(BossVisualController.Part.Boss);
			m_visualController.PlayDefeatSmoke();
			break;
		}
