...
public class RSRGenerator : MonoBehaviour
{
	...
	{
		int testValueAsInt = ABTesting.GetTestValueAsInt(ABTesting.Tests.RSR_RunsBefore1);
		if (testValueAsInt != -1)
		{
			m_runsBeforeFirstRSR = testValueAsInt;
		}
		testValueAsInt = ABTesting.GetTestValueAsInt(ABTesting.Tests.RSR_Spread1);
		if (testValueAsInt != -1)
		{
			m_spreadOfRunsToAppearFirst = testValueAsInt;
		}
		testValueAsInt = ABTesting.GetTestValueAsInt(ABTesting.Tests.RSR_RunsBeforeNext);
		if (testValueAsInt != -1)
		{
			m_runsBetweenRSR = testValueAsInt;
		}
		testValueAsInt = ABTesting.GetTestValueAsInt(ABTesting.Tests.RSR_SpreadNext);
		if (testValueAsInt != -1)
		{
			m_spreadOfRunsToAppear = testValueAsInt;
		}
		testValueAsInt = ABTesting.GetTestValueAsInt(ABTesting.Tests.RSR_MaxDaily);
		if (testValueAsInt != -1)
		{
			m_maxDailyRSR = testValueAsInt;
		}
	}
