...
public class Audio : MonoBehaviour
{
	...
	{
		Transform transform = m_poolAudioSources.FirstOrDefault((Transform source) => source.GetComponent<AudioSource>().clip == clip);
	...
	public static bool IsPlaying(Transform sourceTransform)
	{
		...
		{
			return sourceTransform.gameObject.GetComponent<AudioSource>().isPlaying;
		}
		return false;
	}
	...
	public static void Stop(AudioClip clip)
	{
		...
		{
			Transform sourceTransform = m_poolAudioSources.FirstOrDefault((Transform source) => source.GetComponent<AudioSource>().clip == clip);
	...
	public static void Stop(ref Transform sourceTransform)
	{
		if ((bool)sourceTransform && sourceTransform.gameObject.activeInHierarchy)
		{
			sourceTransform.gameObject.GetComponent<AudioSource>().Stop();
	...
	public static void StopAll()
	{
		...
		{
			poolAudioSource.GetComponent<AudioSource>().Stop();
		}
		m_bPaused = false;
	}
	...
	private void Update()
	{
		...
		if (m_fCleanUpTimer > m_kfCleanUpInterval)
		{
			m_fCleanUpTimer = 0f;
			{
				foreach (Transform poolAudioSource in m_poolAudioSources)
				{
					if (!poolAudioSource.GetComponent<AudioSource>().isPlaying)
					{
	...
	private void PauseAll()
	{
		...
		foreach (Transform poolAudioSource in m_poolAudioSources)
		{
			if (poolAudioSource.GetComponent<AudioSource>().isPlaying)
			{
				poolAudioSource.GetComponent<AudioSource>().Pause();
	...
	private void UnpauseAll()
	{
		foreach (Transform poolAudioSource in m_poolAudioSources)
		{
			...
			{
				poolAudioSource.GetComponent<AudioSource>().volume = m_sfxVolume * m_globalVolume * m_sfxVolumeMod;
				poolAudioSource.GetComponent<AudioSource>().Play();
