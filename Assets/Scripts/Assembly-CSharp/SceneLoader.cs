...
public class SceneLoader : MonoBehaviour
{
	...
	private IEnumerator LoadAdditionalScenes()
	{
		for (int i = 0; i < m_scenesToLoad.Length; i++)
		{
			...
			if (highendScene != SceneIdentifiers.ID.Global && ShouldSceneBeLoaded(highendScene))
			{
			...
			else if (simpleScene != SceneIdentifiers.ID.Global)
			{
	...
	{
		string supportID = string.Format("{0} Scene", scene.ToString());
	...
	private void Event_OnAllAssetsLoaded()
	{
		...
		{
			gameObject.SetActive(GameState.LostWorldEventActive);
		}
	}
}
