...
public class HudContent_FloatingScore
{
	private class PoolEntry
	{
		...
		public Vector3 m_originalSource = Vector3.zero;
	...
	private Vector3 m_sourceOrigin = Vector3.zero;
	...
	{
		m_sourceOrigin = FindFloatingScoreSource(m_floatingScoreSource, m_floatingScoreSourceOffset, m_floatingScorePool[0].m_label);
	}
	...
	private void ResetPool()
	{
		...
		foreach (PoolEntry poolEntry in floatingScorePool)
		{
			...
			poolEntry.m_root.SetActive(false);
		}
		m_currentScoreEntry = 0;
	}
		...
		foreach (PoolEntry poolEntry in floatingScorePool)
		{
			if (!poolEntry.m_free)
			{
				...
				if (poolEntry.m_runningTime >= m_floatingScoreTime)
				{
					...
					poolEntry.m_originalSource = Vector3.zero;
				}
				else
				{
					float num2 = m_floatingScoreSpeed.Evaluate(poolEntry.m_runningTime);
					poolEntry.m_currentOffset += num2 * Time.deltaTime;
					Vector3 originalSource = poolEntry.m_originalSource;
					originalSource.y += poolEntry.m_initialOffset;
					originalSource.y += poolEntry.m_currentOffset;
					poolEntry.m_container.transform.position = originalSource;
				}
				num++;
			}
		}
	...
	{
		UIPlayAnimation uIPlayAnimation = source.GetComponent<UIPlayAnimation>();
		if (uIPlayAnimation == null)
		{
			uIPlayAnimation = source.GetComponentInChildren<UIPlayAnimation>();
		}
		...
		uIPlayAnimation.target = animation;
	}
	...
	private void Event_OnScoreIncreased(int score, ScoreTracker.ScoreNotify notify)
	{
		...
		ShowScoreIncrease(score, iconName, false);
	}

	private void Event_OnPickupRings(int ringCount)
	{
		ShowScoreIncrease(ringCount, "prize-gold-ring", true);
	}
	...
	private void ShowScoreIncrease(int score, string iconName, bool showAsMultiply)
	{
		...
		else
		{
			...
			poolEntry.m_root.SetActive(true);
			poolEntry.m_free = false;
			poolEntry.m_runningTime = 0f;
			poolEntry.m_currentOffset = 0f;
			poolEntry.m_initialOffset = GetInitialOffset() - 0.1f;
			poolEntry.m_originalSource = m_sourceOrigin;
			poolEntry.m_displayedScore = (uint)score;
			poolEntry.m_icon.spriteName = iconName;
			if (iconName != string.Empty)
			{
				poolEntry.m_icon.gameObject.SetActive(true);
				poolEntry.m_background.gameObject.SetActive(true);
				poolEntry.m_icon.spriteName = iconName;
			...
			{
				poolEntry.m_icon.gameObject.SetActive(false);
				poolEntry.m_background.gameObject.SetActive(false);
			}
			if (iconName != string.Empty)
			{
