...
public class CharacterSceneLoader : MonoBehaviour
{
	...
	private void CopyAnimationStates(GameObject sourceObject, GameObject targetObj)
	{
		...
		{
			flag = false;
			foreach (AnimationState item in targetObj.GetComponent<Animation>())
			{
				if ((bool)item)
				{
					targetObj.GetComponent<Animation>().RemoveClip(item.clip);
					flag = true;
					break;
				}
			}
		}
		foreach (AnimationState item2 in sourceObject.GetComponent<Animation>())
		{
			targetObj.GetComponent<Animation>().AddClip(sourceObject.GetComponent<Animation>()[item2.name].clip, item2.name);
		}
	}
