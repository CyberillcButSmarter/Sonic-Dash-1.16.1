...
public class HudContent_DashMeter
{
	...
	public HudContent_DashMeter(DashMeter dashMeter, UISprite filledMeterSprite, GameObject dashModeShowTrigger, GameObject dashModeEnabledTrigger, AudioClip dashMeterBlockedAudio, UITexture dashMeterBar, UISprite dashSprite, UISprite spinningSprite)
	{
		...
		m_dashSprite = dashSprite;
		m_spinningSprite = spinningSprite;
	...
	public void OnResetOnNewGame()
	{
		...
		m_dashSprite.enabled = true;
		m_spinningSprite.enabled = false;
		m_dashMeterBar.gameObject.SetActive(true);
		m_filledMeterSprite.gameObject.SetActive(false);
		HideDashMeter();
	...
	private void ShowDashMeter()
	{
		if (ShouldShowDashMeter())
		{
			...
			{
				m_dashMeterBar.gameObject.SetActive(false);
				m_filledMeterSprite.gameObject.SetActive(true);
			}
			else
			{
				m_dashMeterBar.gameObject.SetActive(true);
	...
	private void Event_OnDashMeterTriggered(bool enabled)
	{
		if (!enabled)
		{
			Audio.PlayClip(m_dashMeterBlockedAudio, false);
			return;
		}
		if ((m_state & State.Filled) == State.Filled)
		{
	...
	{
		m_dashSprite.enabled = false;
		m_spinningSprite.enabled = true;
	}
