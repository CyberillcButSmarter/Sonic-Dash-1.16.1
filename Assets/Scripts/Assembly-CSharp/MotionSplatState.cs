...
public class MotionSplatState : MotionDeathState
{
	...
	public MotionSplatState(SonicHandling handling, SplineTracker tracker, SonicAnimationControl.SplatType splatType)
	{
		...
		m_startSpeed = 0.01f;
		m_startSpeed += ((m_splatType != SonicAnimationControl.SplatType.Stationary) ? (m_tracker.TrackSpeed * Sonic.Handling.SplatRestitution) : 0f);
	}
	...
	public override LightweightTransform CalculateNewTransform(TransformParameters tParams)
	{
		...
		if (m_splatTimer >= num && (((tParams.CurrentTransform.Location - currentSplineTransform.Location).y < 1f) ? true : false))
		{
			DoDeath(true, tParams.StateMachine);
		}
