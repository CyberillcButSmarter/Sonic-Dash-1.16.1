...
public class CameraTypeSonicFollow : CameraType
{
	...
	private bool IsTrackerUsable
	{
		get
		{
			return m_tracker != null && m_tracker.Target != null;
		}
	}
	...
	public override bool EnableSmoothing
	{
		get
		{
			return m_EnableSmoothingOnMainCamera;
		}
	}
	...
	private float CalculateLaneLateralOffset(Track.Lane lane)
	{
		...
		float targetLateralOffset = m_targetLateralOffset;
		float num3;
		switch (lane)
		{
		...
		float num4 = targetLateralOffset * num3;
		return num4 + num2;
	}
	...
	private void LateUpdate()
	{
		...
		Vector3 normalized = vector3.normalized;
		Vector3 vector4 = Vector3.Cross(sonicTransform.Forwards, normalized);
		float num2 = Vector3.Angle(vector4, sonicTransform.Up) * ((!(Vector3.Dot(sonicTransform.Up, normalized) > 0f)) ? 1f : (-1f));
