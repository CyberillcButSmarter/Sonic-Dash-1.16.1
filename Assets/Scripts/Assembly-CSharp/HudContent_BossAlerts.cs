...
public class HudContent_BossAlerts
{
	...
	private void ShowBattleBeginElement()
	{
		...
		Audio.PlayClip(m_alertEnterSound, false);
	...
	private void HideBattleBeginElement()
	{
		...
		{
			m_battleBeginTrigger.SendMessage("OnClick", SendMessageOptions.RequireReceiver);
			Audio.PlayClip(m_alertExitSound, false);
			m_state &= ~State.IntroVisible;
		}
	}
	...
	private void Event_OnBossBattleOutroStart()
	{
		...
		string format = LanguageStrings.First.GetString("BOSS_AWARD_PROMPT");
		m_scoreBonusLabel.text = string.Format(format, LanguageUtils.FormatNumber(m_bossBonusScore));
	...
	private void Event_OnBossBattlePhaseStart(int currentPhase)
	{
		...
		if (phase.Type == BossBattleSystem.Phase.Types.Attack1 && BossLoader.Instance().m_currentBoss == BossLoader.Bosses.Zazz)
		{
			ShowDodgeHint();
		}
	}
