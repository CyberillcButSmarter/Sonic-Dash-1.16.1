...
public class BossFlyBy : MonoBehaviour
{
	...
	private IEnumerator performBehaviour()
	{
		...
		yield return null;
		m_movementController.SetOrientation(m_flybyEndInSonicLocalSpace - m_flybyStartInSonicLocalSpace, true);
		while (timer <= m_flybyProgressCurve.keys[m_flybyProgressCurve.length - 1].time)
		{
			...
			float progress = Mathf.Clamp01(m_flybyProgressCurve.Evaluate(timer));
			Vector3 position = Vector3.Lerp(m_flybyStartInSonicLocalSpace, m_flybyEndInSonicLocalSpace, progress);
			m_movementController.MoveToDestination(position, m_enableBossDrift, false);
