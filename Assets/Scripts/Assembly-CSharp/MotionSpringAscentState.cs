...
public class MotionSpringAscentState : MotionSpringState
{
	...
	public override LightweightTransform CalculateNewTransform(TransformParameters tParams)
	{
		...
		tParams.Physics.UpdateJump(false);
		if (flag)
		{
	...
	private MotionState GetNextState()
	{
		...
		if (m_springType == SpringTV.Type.TokenHunt)
		{
			MotionSpringTokenHuntState.TrackGenState currentTrackState = ((m_trackState != TrackState.Requested) ? MotionSpringTokenHuntState.TrackGenState.Generated : MotionSpringTokenHuntState.TrackGenState.Requested);
			return new MotionSpringTokenHuntState(currentTrackState, Sonic.Handling, base.Physics);
		}
		MotionSpringGesturesState.TrackGenState currentTrackState2 = ((m_trackState != TrackState.Requested) ? MotionSpringGesturesState.TrackGenState.Generated : MotionSpringGesturesState.TrackGenState.Requested);
		return new MotionSpringGesturesState(currentTrackState2, Sonic.Handling, base.Physics);
	}
