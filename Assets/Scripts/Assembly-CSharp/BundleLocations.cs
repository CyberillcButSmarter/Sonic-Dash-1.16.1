...
{
	private const string DownloadLocation = "http://data.sd.hla.sega.co.uk/asset bundles";

	private const string SpaceReplacement = "+";

	public static string GetVersionedPath(string assetName, string extension)
	{
		return GetReferencedPath(assetName, extension, "1.16.1");
	}
	...
	public static string GetReferencedPath(string assetName, string extension, string folder)
	{
		...
		string arg = string.Format("{0}-{1}.{2}", assetName, platformIdentifier, extension);
		string text = string.Format("{0}/{1}/{2}", "http://data.sd.hla.sega.co.uk/asset bundles", folder, arg);
