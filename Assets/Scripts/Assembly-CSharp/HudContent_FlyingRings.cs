...
public class HudContent_FlyingRings
{
	...
	private void UpdateItems()
	{
		for (int i = 0; i < m_itemsPool.Length; i++)
		{
			if (m_itemsPool[i].m_free)
			{
				m_itemsPool[i].m_manaSource.gameObject.SetActive(false);
				m_itemsPool[i].m_ringSource.gameObject.SetActive(false);
				continue;
			}
			m_itemsPool[i].m_manaSource.gameObject.SetActive(true);
			m_itemsPool[i].m_ringSource.gameObject.SetActive(true);
			...
			Vector3 position = ((m_itemsPool[i].m_content != Content.Ring) ? m_itemsPool[i].m_sprite.transform.position : m_itemsPool[i].m_texture.transform.position);
	...
	private void ReleaseItems()
	{
		...
		for (int i = 0; i < num2; i++)
		{
			...
			if ((m_state & States.Banking) == States.Banking)
			{
				...
				BuildTrajectory(m_currentEntry, true);
