...
public class TrackGenerator : Track
{
	...
	private int m_iLayerDefault = -1;

	private int m_iLayerTrack = -1;

	private Camera m_mainCamera;

	private int m_iAllTrackLayersMask;

	private int m_iTrackWaterReflectionMask;
	...
	public override bool IsAvailable
	{
		get
		{
			return m_pieces != null;
		}
	}

	public override Spline StartSpline
	{
		get
		{
			return m_pieces.Peek().MiddleSpline;
		}
	}
	...
	public bool GameplayMorePiecesRequired
	{
		get
		{
			return m_generationParams == null || m_generatedLength < m_generationParams.GameplayMinTrackDistanceToGenerate;
		}
	}

	public bool IsTrackGenerated
	{
		get
		{
			return m_trackIsGenerated;
		}
	}

	public float DifficultyRelevantLengthAtTrackStart
	{
		get
		{
			return m_trackStartDifficultyRelevantLength;
		}
	}

	public TrackGenerationParameters GenerationParams
	{
		get
		{
	...
	{
		CanHaveBossBattle = (BossLoader.ZazzBossActive || BossLoader.EggmanBossActive) && !TutorialSystem.instance().isTrackTutorialEnabled();
		if (GrantooManager.GetInstance().IsActive())
		{
		...
		if (m_numBossSpawns == 0)
		{
			...
			if (PlayerStats.GetCurrentStats().m_trackedStats[92] == 0)
			{
	...
	private void Event_RequestNewTrackMidGame(MidGameNewTrackRequest request)
	{
		...
		TutorialSystem.instance().Reset(false);
	...
	private void InitialiseGeneration()
	{
		...
		ResetBossBattle();
		TutorialSystem.instance().Reset(true);
	}
	...
	private IEnumerator TrackCreationKickoff(TrackCreationFlags flags, SpringTV.Type springType, int forcedSubzone, float emptyAirPrefix)
	{
		...
		creationCoroutineFlags = (CreateFlags)((int)creationCoroutineFlags | ((springType == SpringTV.Type.SetPiece) ? 2 : 0));
		StartCoroutine(ProcessDecorationQueue());
		yield return StartCoroutine(TrackCreator(creationCoroutineFlags, emptyAirPrefix));
		if (isChangingZone)
		{
			yield return StartCoroutine(HDTextureLoader.ReplaceHDVariants("HD Textures - Track", false, false));
		}
	...
	public override SplineUtils.SplineParameters GetSplineToSideOf(Spline fromSpline, LightweightTransform fromTransform, SideDirection toDirection)
	{
		...
		return new SplineUtils.SplineParameters
		{
			Target = spline,
	...
	private IEnumerator TrackCreator(CreateFlags flags, float emptyAirPrefix)
	{
		...
		{
			yield return StartCoroutine(AddPieceToTrack(TrackPieceSequence.GenericPieceType.EmptyAir, emptyAirPrefix, false));
		}
		...
		TrackSegment firstSegment = CreateNextSegment(firstSegmentPrefab, 1f, false);
		...
		{
			yield return StartCoroutine(AddPieceToTrack(TrackPieceSequence.GenericPieceType.Straight, 1f, false));
		}
		...
		if (isTutorialEnabled)
		{
			...
			for (int templateIndex = startIndex; templateIndex <= endIndex; templateIndex++)
			{
				...
				yield return StartCoroutine(AddTemplateToTrack(gameplayTemplate, true));
			}
		}
		else if (isBossBattle)
		{
			...
			while (true)
			{
				...
				while (phaseTrackDistance >= phaseMinTrackDistance)
				{
					...
					GameplayTemplateGenerator.TemplateInstantiator gameplayTemplate2 = m_gameplayTemplateGenerator.CreateTemplate(GameplayTemplate.Group.FixedStart, m_gameplayStraightTemplatesGenerated, m_difficultyRelevantTrackLength, m_excludedTemplates);
					yield return StartCoroutine(AddTemplateToTrack(gameplayTemplate2, false));
					yield return StartCoroutine(FillRemainingTrackWithTemplate(gameplayTemplate2.EndPosition));
					phase = null;
					break;
				}
				if (phase == null)
				...
				{
					int startIndex2 = BossBattleSystem.Instance().GetStartTemplateIndexForPhase(phaseIndex);
					int templateCount = BossBattleSystem.Instance().GetTemplateCountForPhase(phaseIndex);
					int templateIndex2 = startIndex2;
					while (phaseTrackDistance < phaseMinTrackDistance)
					{
						GameplayTemplateGenerator.TemplateInstantiator gameplayTemplate3 = m_gameplayTemplateGenerator.CreateBossBattleTemplate(templateIndex2);
						yield return StartCoroutine(AddTemplateToTrack(gameplayTemplate3, true));
						templateIndex2 = startIndex2 + (templateIndex2 - startIndex2 + 1) % templateCount;
						phaseTrackDistance += m_generatedLength - straightStartDistance;
						straightStartDistance = m_generatedLength;
					}
				}
				else
				{
					...
					{
						GameplayTemplateGenerator.TemplateInstantiator gameplayTemplate4 = m_gameplayTemplateGenerator.CreateTemplate(GameplayTemplate.Group.FixedStart, m_gameplayStraightTemplatesGenerated, m_difficultyRelevantTrackLength, m_excludedTemplates);
						m_gameplayStraightTemplatesGenerated++;
						yield return StartCoroutine(AddTemplateToTrack(gameplayTemplate4, false));
					}
				}
		...
		else
		{
			...
			while (true)
			{
				...
				float straightStartDistance2 = m_generatedLength;
				while (m_generatedLength - straightStartDistance2 < m_generationParams.MinStraightLength)
				{
					if (straightsToNextSetPiece < 0 && remainingSetPieces > 0)
					{
						yield return StartCoroutine(AddPieceToTrack(TrackPieceSequence.GenericPieceType.Straight, 1f, TrackPieceSequence.Flags.ForceToLowElevation, false));
					...
					{
						GameplayTemplateGenerator.TemplateInstantiator gameplayTemplate5 = m_gameplayTemplateGenerator.CreateTemplate(GameplayTemplate.Group.FixedStart, m_gameplayStraightTemplatesGenerated, m_difficultyRelevantTrackLength, m_excludedTemplates);
						m_gameplayStraightTemplatesGenerated++;
						yield return StartCoroutine(AddTemplateToTrack(gameplayTemplate5, false));
					}
				}
				...
				foreach (TrackDatabase.TrackPiecePrefab cornerPiecePrefab2 in m_trackSequence.NextCorner())
				{
					yield return StartCoroutine(AddCornerPiece(cornerPiecePrefab2));
				}
				m_isStraightOpen = false;
			}
			GameplayTemplateGenerator.TemplateInstantiator gameplayTemplate6 = m_gameplayTemplateGenerator.CreateTemplate(GameplayTemplate.Group.FixedStart, m_gameplayStraightTemplatesGenerated, m_difficultyRelevantTrackLength, m_excludedTemplates);
			yield return StartCoroutine(AddTemplateToTrack(gameplayTemplate6, false));
			yield return StartCoroutine(FillRemainingTrackWithTemplate(gameplayTemplate6.EndPosition));
		}
		...
		yield return StartCoroutine(AddTemplateToTrack(springTemplate, false));
		yield return StartCoroutine(AddPieceToTrack(TrackPieceSequence.GenericPieceType.TrackCap, 1f, false));
		yield return StartCoroutine(AddPieceToTrack(TrackPieceSequence.GenericPieceType.EmptyAir, 1f, true));
		EnableNextGameplayStraight();
		while (m_segmentEnabler.IsProcessingPending)
		{
			yield return null;
	...
	{
		TrackSegment nextCornerPiece = CreateNextSegment(cornerPiecePrefab, 1f, false);
	...
	private void EnableNextGameplayStraight()
	{
		if (m_isTrackStraightLoddingEnabled)
		{
			...
			{
				trackSegment3.SetGameplayEnabled(true);
	...
	private IEnumerator AddTemplateToSetPiece(GameplayTemplateGenerator.TemplateInstantiator templateMaker)
	{
		...
		yield return StartCoroutine(AddPieceToTrack(TrackPieceSequence.GenericPieceType.SetPiece, 1f, false));
	...
	private IEnumerator AddTemplateToTrack(GameplayTemplateGenerator.TemplateInstantiator templateMaker, bool forceFlat)
	{
		...
		while (templateMaker.InProgress)
		{
			...
			yield return StartCoroutine(AddPieceToTrack(nextPieceType, nextPieceStretch, sequenceFlags, false));
	...
	private IEnumerator ProcessDecorationQueue()
	{
		...
		while (true)
		{
			...
			Lane? currentGapLane = ((!isGapPiece) ? ((Lane?)null) : new Lane?(TrackDatabase.GetGapLane(templateType)));
			...
			SetPieceLayerRecursive(segmentToDecorate.transform);
	...
	private TrackSegment CreateNextSegment(TrackDatabase.TrackPiecePrefab pieceType, float stretchMultiplier, bool endSection)
	{
		...
		SetPieceLayerRecursive(gameObject2.transform);
	...
	private void EnableCollisionOnSegmentSequence(TrackSegment newSegment)
	{
		...
		TrackSegment thisSegment2 = ((segmentIndex >= m_pieces.Count - 1) ? null : m_pieces.ElementAt(segmentIndex + 1));
	...
	private void CreateRandomNumberGenerator()
	{
		...
		if (!IsSeedFixed && GrantooManager.GetInstance().IsActive())
		{
			Seed = GrantooManager.GetInstance().GetCurrentSeed();
	...
	public void InitTrackHider()
	{
		m_iLayerDefault = LayerMask.NameToLayer("Default");
		m_iLayerTrack = LayerMask.NameToLayer("Trackpieces");
		CameraTypeMain cameraTypeMain = UnityEngine.Object.FindObjectOfType(typeof(CameraTypeMain)) as CameraTypeMain;
		m_mainCamera = cameraTypeMain.GetComponentInChildren<Camera>();
		m_iTrackWaterReflectionMask = (1 << LayerMask.NameToLayer("Track Collision")) + (1 << LayerMask.NameToLayer("Reflect"));
		m_iAllTrackLayersMask = (1 << LayerMask.NameToLayer("Trackpieces")) + (1 << LayerMask.NameToLayer("Track Collision")) + (1 << LayerMask.NameToLayer("Powerup Collision")) + (1 << LayerMask.NameToLayer("shoreline")) + (1 << LayerMask.NameToLayer("Enemies")) + (1 << LayerMask.NameToLayer("Obstacles")) + (1 << LayerMask.NameToLayer("Reflect"));
	}

	public void SetPieceLayerRecursive(Transform t)
	{
		if (t.gameObject.layer == m_iLayerDefault)
		{
			t.gameObject.layer = m_iLayerTrack;
		}
		int childCount = t.childCount;
		for (int i = 0; i < childCount; i++)
		{
			SetPieceLayerRecursive(t.GetChild(i));
		}
	}

	public void HideTrack(bool bHide)
	{
		GameObject gameObject = GameObject.FindGameObjectWithTag("WaterTop");
		m_water = ((!(gameObject != null)) ? null : gameObject.GetComponent<Water>());
		int cullingMask = m_mainCamera.cullingMask;
		if (!bHide)
		{
			cullingMask |= m_iAllTrackLayersMask;
			if (m_water != null)
			{
				Water water = m_water;
				water.m_ReflectLayers = (int)water.m_ReflectLayers | m_iTrackWaterReflectionMask;
			}
		}
		else
		{
			...
			if (m_water != null)
			{
				Water water2 = m_water;
				water2.m_ReflectLayers = (int)water2.m_ReflectLayers & ~m_iTrackWaterReflectionMask;
			}
		}
