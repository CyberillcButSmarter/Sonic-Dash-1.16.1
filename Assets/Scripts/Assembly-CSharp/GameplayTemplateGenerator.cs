...
public class GameplayTemplateGenerator : MonoBehaviour
{
	...
	public class TemplateInstantiator
	{
		...
		public GameplayTemplate GameplayTemplate
		{
			get
			{
				return m_templateCreating;
			}
		}
		...
		public bool InProgress
		{
			get
			{
				return m_currentRow < m_templateCreating.RowCount;
			}
		}
		...
		public IEnumerator AddGameplayTo(SplineTracker segmentTracker, TrackSegment segment, uint excludedEntities)
		{
			...
			Track.Lane? gapLane = ((!isAnyLaneMissing) ? ((Track.Lane?)null) : new Track.Lane?(TrackDatabase.GetGapLane(currentSegment.Template.PieceType.Type)));
			...
			while (true)
			{
				...
				if (!row.IsEventRow)
				{
					...
					{
						if (lane != GameplayTemplate.Lane.LeftFish && lane != GameplayTemplate.Lane.RightFish)
						{
		...
		private void FillTrackWithChoppers(SplineTracker segmentTracker, float segmentTrackPosition, Transform chopperContainer)
		{
			...
			while (num3 > 0)
			{
				GameplayTemplate.Lane lane = ((sideDirection != SideDirection.Left) ? GameplayTemplate.Lane.RightFish : GameplayTemplate.Lane.LeftFish);
		...
		private GameObject CreateSuitableEntity(GameplayTemplate.Entity entityToGenerate, Spline spline, LightweightTransform cellTransform, LightweightTransform laneTransform, float trackDistance, GameplayTemplate.Lane lane, bool isAnyLaneMissing, uint excludedEntities)
		{
			...
			Track.Lane lane2 = (Track.Lane)num;
			if (((uint)type & excludedEntities) == 0)
			{
				...
				default:
				{
					if ((type & (TrackEntity.Kind)165888) != TrackEntity.Kind.Invalid)
					{
		...
		private GameObject GenerateObstacle(GameplayTemplate.Entity entityToGenerate, Track.Lane lane, float trackDistance, LightweightTransform laneTransform, Spline spline, bool isAnyLaneMissing)
		{
			...
			{
				TrackEntity.Kind kind = (TrackEntity.Kind)(type & 0xF0);
	...
	public GameplayTemplateParameters Parameters
	{
		get
		{
			return m_parameters;
	...
	{
		Func<GameplayTemplate, bool> customIsSuitable = (GameplayTemplate gameplayTemplate) => !gameplayTemplate.HasGaps;
		...
		IEnumerable<GameplayTemplate> enumerable = allTemplatesForDistance.Where((GameplayTemplate gameplayTemplate) => gameplayTemplate.RowCount <= maxRowCount && gameplayTemplate.RowCount >= minRowCount);
		if (!enumerable.Any())
		{
		...
		GameplayTemplate template = m_database.PickRandomTemplate(enumerable, m_rng, null);
	...
	private GameObject CreateObstacle(TrackEntity.Kind obstacleType, Spline spline, LightweightTransform worldTransform, ref int desiredLaneCount)
	{
		...
		if ((obstacleType & (TrackEntity.Kind)571) != TrackEntity.Kind.Invalid)
		{
			obstacleCharacteristics |= 1u;
		}
		if ((obstacleType & (TrackEntity.Kind)98) != TrackEntity.Kind.Invalid)
		{
		...
		switch (obstacleType)
		{
		case TrackEntity.Kind.SpikePit:
			typeFromHandle = typeof(ObstacleSpikes);
			break;
		case TrackEntity.Kind.Mine:
			typeFromHandle = typeof(ObstacleMine);
			break;
		default:
			typeFromHandle = typeof(Wall);
			break;
		}
		Type desiredType = typeFromHandle;
		...
		IEnumerable<Obstacle> source = from transform in spawnPool.prefabs.Values
			select transform.GetComponent<Obstacle>() into obstacle2
			where obstacle2 != null
			select obstacle2;
		...
		IEnumerable<Obstacle> enumerable = source2.Where((Obstacle obstacle2) => obstacle2.NumLanesOccupied == bestLaneCount);
		if (!enumerable.Any())
		{
			enumerable = source2.Where((Obstacle obstacle2) => obstacle2.NumLanesOccupied == 1);
			desiredLaneCount = 1;
		}
		...
		Obstacle obstacle = source3.ElementAt(m_rng.Next(source3.Count()));
		Transform prefab = obstacle.transform;
		return m_obstacleGenerator.GenerateObstacle(prefab, worldTransform, Track, spline);
	}
	...
	private GameObject CreateGroundEnemy(TrackEntity.Kind enemyType, Enemy.Direction dir, Spline spline, LightweightTransform worldTransform, float trackDistance)
	{
		...
		dir = ((dir != Enemy.Direction.TowardsPlayer && dir != Enemy.Direction.AwayFromPlayer) ? dir : Enemy.Direction.Stationary);
	...
	private void CalculateNextSpringType(Spring newSpring, SpringTV newTV, TrackSegment segment, Track.Lane trackLane)
	{
		...
		SpringTV.Destination destination;
		switch (trackGenerator.CalculateCurrentSubzoneIndex())
		{
		case 0:
			destination = SpringTV.Destination.Grass;
			break;
		case 1:
			destination = SpringTV.Destination.Temple;
			break;
		default:
			destination = SpringTV.Destination.Beach;
			break;
		}
		bool flag = false;
		...
		if (trackGenerator.CanHaveBossBattle)
		{
			...
			type = SpringTV.Type.BossStart;
			flag = true;
		}
		else if (bossBattleSystem != null && bossBattleSystem.IsEnabled() && bossBattleSystem.GetCurrentPhase() == 0)
		{
			type = SpringTV.Type.BossEnd;
			flag = true;
		}
		...
		else if (EspioSpecialEvent.IsEventActive() && (float)m_rng.NextDouble() < 0.25f && !DCs.ChallengeSpringCreated)
		{
			DCs.ChallengeSpringCreated = true;
			type = SpringTV.Type.TokenHunt;
			flag = true;
		}
		...
		else
		{
			...
			float num3 = 0f;
			empty = string.Format(" Rnd = {0}%", (int)(num2 * 100f));
