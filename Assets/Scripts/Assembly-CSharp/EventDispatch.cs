...
public class EventDispatch
{
	...
	{
		List<ObjectProperties> list = FindObjectList(eventName, false);
		if (list != null)
		{
	...
	{
		List<ObjectProperties> list = FindObjectList(eventName, true);
	...
	public static void UnregisterInterest(string eventName, object uninterestedObject)
	{
		...
		if (list != null)
		{
			list.RemoveAll((ObjectProperties thisElement) => uninterestedObject == thisElement.m_interestedObject);
		}
	}
	...
	{
		return string.Format("Event_{0}", eventName);
	}
}
