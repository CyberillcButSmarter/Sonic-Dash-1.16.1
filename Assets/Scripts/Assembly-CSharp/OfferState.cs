...
public class OfferState : MonoBehaviour
{
	...
	{
		FileDownloader fDownloader = new FileDownloader(FileDownloader.Files.OfferState, true);
	...
	public static bool CanDisplay()
	{
		...
		if (offer.m_nonPayingUsers)
		{
			...
			if (currentStats.m_trackedStats[74] > 0)
			{
				return false;
			}
		}
	...
	private void Event_ProvideContent(string storeID, string transactionID, int quantity, ProvideContentSource contentSource, int contentRewardReason, StorePurchases.ShowDialog showRewardDialog)
	{
	...
	private void Event_OnGameDataLoaded(ActiveProperties activeProperties)
	{
		...
		if (m_loadCount > 1)
		{
			...
			string text = activeProperties.GetString("OfferState_LastDisplayTS");
			if (text != null && text.Length > 4)
			{
				m_currentOffer.m_lastDisplayTS = DateTime.Parse(text);
			}
