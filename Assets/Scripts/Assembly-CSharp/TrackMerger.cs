	...
	public IEnumerator performStaticBatching()
	{
		...
		foreach (MeshFilter filter in meshFilters)
		{
			if (!(filter.gameObject.tag == "MovableShadow") && !(filter.gameObject.tag == "Dynamic"))
			{
				...
				if (Mathf.Approximately(scale.x, scale.y) && Mathf.Approximately(scale.y, scale.z) && (bool)filter.GetComponent<Renderer>() && (bool)filter.gameObject.GetComponent<Renderer>().sharedMaterial && (bool)filter.sharedMesh)
				{
