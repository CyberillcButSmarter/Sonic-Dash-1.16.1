...
public class RingRender : MonoBehaviour
{
	...
	private void UpdateMagnetism()
	{
		...
		bool flag2 = DashMonitor.instance().isDashing();
		bool flag3 = HeadstartMonitor.instance().isHeadstarting();
		bool flag4 = flag || flag2 || flag3;
		if (ABTesting.GetTestValueAsInt(ABTesting.Tests.PLAY_MagnetDash) == 0)
		{
			flag4 = flag;
		}
		...
		foreach (RingSequence ringSequence in sequences)
		{
			...
			for (int j = 0; j < ringSequence.Length; j++)
			{
				...
				if (ring.isMagnetised)
				{
					...
					if (!(transform == null))
					{
						Vector3 vector = transform.TransformPoint(ring.m_preMagnetisedLocalPosition);
						ring.m_position = Vector3.Lerp(vector, position, num4);
	...
	{
		if (null == m_worldContainerTransform || null == Sonic.Transform || (m_mainCamera.GetCullingMask() & (1 << base.gameObject.layer)) == 0)
		{
			return;
		}
		bool flag = false;
	...
	private void PlayPickUpAudio(RingSequence.Ring thisRing)
	{
		...
		{
			Audio.PlayClip(m_pickUpAudioClipAlt, false, m_pickupSfxPitch, 0.4f);
		}
		Audio.PlayClip(m_pickUpAudioClip, false, 1f, 1f);
	}
}
