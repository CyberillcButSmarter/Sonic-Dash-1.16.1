...
public class MissionUi_PageContent : MonoBehaviour
{
	...
	private void Update()
	{
		if (m_swapMissionsWhenComplete && m_allowSwap)
		{
			...
			if (!(m_swapBannerDelay > 0f) && !(m_totalProgression.value < 1f))
			{
	...
	private void PopulateOnStart()
	{
		...
		m_multiplier.text = string.Format("{0}", ScoreTracker.NextMultiplier);
		m_rewardRSR.text = string.Format("{0}", MissionTracker.RSRRewardPerSet);
	...
	{
		m_progressionGroup.SetActive(false);
	}
	...
	private void SetProgressProperties(float progress)
	{
		...
		m_totalProgression.value = progress;
		m_percentageProgression.text = string.Format("{0}%", Mathf.FloorToInt(progress * 100f).ToString());
	}
	...
	private void MoveToNextMissionSet()
	{
		...
		m_multiplier.text = string.Format("{0}", ScoreTracker.NextMultiplier);
		Audio.PlayClip(m_clipIncMulitplier, false);
		if (ABTesting.GetTestValueAsInt(ABTesting.Tests.MSG_MissionBenefits) != 0 && missionSet == 0)
		{
	...
	{
		float initialProgress = m_totalProgression.value;
	...
	private IEnumerator DecreaseProgressBar()
	{
		...
		while (delay < 0.5f);
		float currentTime = 0f;
		float intialProgress = m_totalProgression.value;
		do
		{
			currentTime += 1f / m_sliderCompleteSpeed * IndependantTimeDelta.Delta;
