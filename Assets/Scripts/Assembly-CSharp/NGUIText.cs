...
public static class NGUIText
{
	...
	public static void Align(BetterList<Vector3> verts, int indexOffset, float printedWidth)
	{
		...
		{
			float num14 = (float)rectWidth - printedWidth;
			if (!(num14 < 0f))
			{
				for (int j = indexOffset; j < verts.size; j++)
				{
					verts.buffer[j].x += num14;
				}
			}
			break;
		}
		...
		{
			float num11 = ((float)rectWidth - printedWidth) * 0.5f;
			if (!(num11 < 0f))
			{
				int num12 = Mathf.RoundToInt((float)rectWidth - printedWidth);
				int num13 = Mathf.RoundToInt(rectWidth);
				bool flag = (num12 & 1) == 1;
				bool flag2 = (num13 & 1) == 1;
				if ((flag && !flag2) || (!flag && flag2))
				{
					num11 += 0.5f * fontScale;
				}
				for (int i = indexOffset; i < verts.size; i++)
				{
					verts.buffer[i].x += num11;
				}
			}
			break;
		}
