...
public class BundleState : MonoBehaviour
{
	...
	public List<Bundle> GetValidLocalBundles(out string bundleVersion)
	{
		...
		VersionIdentifiers.VersionStatus versionStatus = VersionIdentifiers.CheckVersionNumbers("1.16.1", stringValue, VersionIdentifiers.Components.Point);
		if (versionStatus == VersionIdentifiers.VersionStatus.Lower)
		{
			Caching.ClearCache();
		}
		if (versionStatus != VersionIdentifiers.VersionStatus.Equal)
		{
			return null;
		}
	...
	public void SaveBundleState()
	{
		if (m_pendingBundles != null && m_pendingBundles.Count != 0)
		{
			...
			array[0].m_properties[0].m_value = "1.16.1";
