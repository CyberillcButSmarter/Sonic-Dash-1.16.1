...
public class Dialog_BoosterHelp : MonoBehaviour
{
	...
	private void SetLocalisedLabel(UILabel descriptionLabel, PowerUps.Type boosterType, string stringId)
	{
		...
		{
			string format2 = LanguageStrings.First.GetString(stringId);
			string text2 = string.Format(format2, Boosters.ScoreMultiplier * 100f - 100f);
		...
		{
			string format = LanguageStrings.First.GetString(stringId);
			string text = string.Format(format, Boosters.GoldenEnemyScoreMultipler);
