...
public class Dialog_BoosterBreadcrumb : MonoBehaviour
{
	...
	[SerializeField]
	private UISprite m_okGlow;

	[SerializeField]
	private UISprite m_okBackground;
	...
	private void OnDisable()
	{
		...
		m_okLabel.gameObject.SetActive(false);
		m_okGlow.gameObject.SetActive(false);
		m_okBackground.gameObject.SetActive(false);
		m_okIcon.gameObject.SetActive(false);
	}
	...
	private void UpdateContent()
	{
		...
		{
			m_titleObject.text = LanguageStrings.First.GetString(string.Format("BOOSTERS_TITLE_{0}", m_boosterName));
			if (m_boosterName == "SCORE_BOOSTER")
			{
				string format = LanguageStrings.First.GetString(string.Format("BOOSTERS_DESC_{0}", m_boosterName));
				string text = string.Format(format, Boosters.ScoreMultiplier * 100f - 100f);
				m_descriptionObject.text = text;
			}
			else if (m_boosterName == "GOLDEN_BADNIK")
			{
				string format2 = LanguageStrings.First.GetString(string.Format("BOOSTERS_DESC_{0}", m_boosterName));
				string text2 = string.Format(format2, Boosters.GoldenEnemyScoreMultipler);
				m_descriptionObject.text = text2;
			}
			else
			{
				m_descriptionObject.text = LanguageStrings.First.GetString(string.Format("BOOSTERS_DESC_{0}", m_boosterName));
			}
