...
internal class EnemyExplosion : MonoBehaviour
{
	...
	public void OnDeath(object[] onDeathParams)
	{
		...
		Component[] componentsInChildren = base.transform.GetComponentsInChildren(typeof(Renderer), false);
	...
	private ParticleSystem FireParticleFX(ParticleSystem fxPrefab, SonicSplineTracker sonicSplineTracker, Vector3 forwardsImpulse)
	{
		...
		if ((bool)transform)
		{
			...
			ParticleInfo item = new ParticleInfo
			{
				...
				m_time = fxPrefab.duration + fxPrefab.startLifetime,
				m_impulse = forwardsImpulse
			};
