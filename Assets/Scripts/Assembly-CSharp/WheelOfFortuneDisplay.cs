...
public class WheelOfFortuneDisplay : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (m_isSpinning)
		{
			...
			if (num2 != num3)
			{
				m_prizeHighlight[num2].SetActive(false);
				m_prizeHighlight[num3].SetActive(true);
				Audio.PlayClip(m_spinSound, false);
			}
		...
		if (!m_isSpinning)
		{
			if ((DCTimeValidation.TrustedTime || WheelOfFortuneSettings.Instance.FirstFreeSpinAvailable) && WheelOfFortuneSettings.Instance.HasFreeSpin)
			{
				if (!m_freeSpinButton.activeSelf || (!m_freeSpinButton.activeSelf && !m_paidSpinButton.activeSelf))
				{
					...
					m_freeSpinButton.SetActive(true);
					m_paidSpinButton.SetActive(false);
				}
			}
			else if (m_freeSpinButton.activeSelf || (!m_freeSpinButton.activeSelf && !m_paidSpinButton.activeSelf))
			{
				...
				m_freeSpinButton.SetActive(false);
				m_paidSpinButton.SetActive(true);
			}
			if (!m_cyclingJackpot)
			{
				m_homeButtonIcon.SetActive(true);
				m_homeButtonSpinner.SetActive(false);
			}
			else
			{
				m_homeButtonIcon.SetActive(false);
				m_homeButtonSpinner.SetActive(true);
			}
		}
		else
		{
	...
	public void Trigger_StopJackpot()
	{
		...
		{
			m_forceStopJackpotCycle = true;
			Audio.PlayClip(m_forceStopPressSound, false);
		}
	}
	...
	private void SpinTheWheel()
	{
		...
		m_buttonSpinner.SetActive(true);
	...
	private IEnumerator EndSpin()
	{
		...
		m_isSpinning = false;
		m_buttonSpinner.SetActive(false);
		m_spinParticle.Stop();
		if (m_isJackpot)
		{
			...
			Audio.PlayClip(m_jackpotPreRewardSound, false);
		...
		else
		{
			...
			Audio.PlayClip(m_normalPreRewardSound, false);
	...
	{
		if (m_awardedPrizeIndex != -1 && m_acceptAward)
		{
			...
			if (!m_isJackpot)
			{
				...
				Audio.PlayClip(m_normalRewardSound, false);
	...
	private void UpdateTimeRemaining()
	{
		...
		{
			m_warningTimeArea.SetActive(true);
			m_safeTimeArea.SetActive(false);
		}
		else
		{
			m_warningTimeArea.SetActive(false);
			m_safeTimeArea.SetActive(true);
		}
	}
	...
	private IEnumerator StartPendingActivation()
	{
		...
		m_prizeHighlight[AngleToPrize(m_currentAngle)].SetActive(false);
		m_prizeHighlight[0].SetActive(true);
		m_currentAngle = 0f;
		...
		for (int i = 1; i < 8; i++)
		{
			PopulateWheelDisplay(i, WheelOfFortuneRewards.Instance.GetRandomPrize(out quantityToAward, false), quantityToAward);
		}
	}
	...
	private IEnumerator PickJackpotPrize()
	{
		...
		do
		{
			finalChoice = WheelOfFortuneRewards.Instance.GetRandomPrize(out finalChoiceQuant, true);
		}
		...
		for (int i = 0; i < m_numberOfJackpotPrizeCycles; i++)
		{
			...
			Audio.PlayClip(m_cycleJackpotSound, false);
			yield return new WaitForSeconds(cycleTime);
		}
		PopulateWheelDisplay(0, finalChoice, finalChoiceQuant);
		Audio.PlayClip(m_replaceJackpotSound, false);
		if (!m_isSpinning)
		{
			for (int j = 0; j < m_blockers.Length; j++)
			{
				m_blockers[j].Blocked = false;
			}
		}
	...
	private void RestoreJackpot()
	{
		...
		if (WheelOfFortuneRewards.Instance.CachedJackpotID != null && WheelOfFortuneRewards.Instance.CachedJackpotID != string.Empty)
		{
			...
			if (flag)
			{
				WheelOfFortuneRewards.Instance.GetRandomPrize(out quantityOut, true);
		...
		PopulateWheelDisplay(0, WheelOfFortuneRewards.Instance.GetRandomPrize(out quantityOut, true), quantityOut);
