...
public class BossFlyOff : MonoBehaviour
{
	...
	private IEnumerator performBehaviour()
	{
		...
		m_movementController.MoveToDestination(parameters, m_enableBossDrift, false);
		...
		{
			float progress = Mathf.Clamp01(m_escapeCurve.Evaluate(timer));
			Vector3 pos = Vector3.Lerp(desiredPositionOffSpline, m_escapeDestination, progress);
			m_movementController.MoveToDestination(pos, false, false);
		}
	}
