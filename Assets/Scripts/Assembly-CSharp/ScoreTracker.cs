...
public class ScoreTracker : MonoBehaviour
{
	...
	public static bool HighScoreAchived
	{
		get
		{
			return (s_scoreTracker.m_state & State.HighScore) == State.HighScore;
		}
	}
	...
	public static long CurrentScore
	{
		get
		{
			return s_scoreTracker.m_currentScore;
		}
	}

	public static long CurrentBoosterBonusScore
	{
		get
		{
			return s_scoreTracker.m_boosterBonusScore;
		}
	}
	...
	public static long HighScore
	{
		get
		{
			return (!(s_scoreTracker == null)) ? s_scoreTracker.m_highestScore : 0;
		}
	}
	...
	public static long PreviousHighScore
	{
		get
		{
			return s_scoreTracker.m_previousHighScore;
		}
	}

	public static uint DashMultiplier
	{
		get
		{
			return s_scoreTracker.m_dashMultiplier;
		}
	}

	public static uint BaseMultiplier
	{
		get
		{
			return s_scoreTracker.m_baseMultiplier;
		}
	}

	public static uint RunMultiplier
	{
		get
		{
			return s_scoreTracker.m_runMultiplier;
		}
	}

	public static uint CurrentMultiplier
	{
		get
		{
			return s_scoreTracker.m_currentMultiplier;
		}
	}

	public static uint NextMultiplier
	{
		get
		{
			return s_scoreTracker.m_baseMultiplier + 1;
		}
	}

	public static int EventBonusMultiplier
	{
		get
		{
			return s_scoreTracker.m_eventBonusMultiplier;
		}
	}
	...
	public static string HighestScoreSavedProperty
	{
		get
		{
	...
	private static string MultiplierSavedProperty
	{
		get
		{
			return "Multiplier";
	...
	private void Event_OnGameFinished()
	{
		...
		{
			m_boosterBonusScore = (int)((float)m_currentScore * Boosters.ScoreMultiplier);
			m_currentScore += m_boosterBonusScore;
			PlayerStats.IncreaseLongStat(PlayerStats.LongStatNames.Score_BoosterEndRunBonus_Total, m_boosterBonusScore);
		}
	...
	private void Event_OnBossHit(int initialscore)
	{
		...
		if (num >= 3 && Boosters.IsBoosterSelected(PowerUps.Type.Booster_EnemyComboBonus))
		{
			...
			PlayerStats.IncreaseLongStat(PlayerStats.LongStatNames.Score_BoosterEnemyCombos_Total, num2);
			if (CharacterManager.Singleton.GetCurrentCharacter() == Characters.Type.Espio)
			{
				PlayerStats.IncreaseLongStat(PlayerStats.LongStatNames.Score_BoosterEnemyCombosAsEspio_Total, num2);
			}
		}
	}
	...
	private void Event_OnEnemyKilled(Enemy enemy, Enemy.Kill killType)
	{
		...
		{
			int num4 = (int)((float)num * Boosters.EnemyComboScoreMultiplier);
			notify = ScoreNotify.EnemyComboBonus;
			EventDispatch.GenerateEvent("OnBoosterActivated", PowerUps.Type.Booster_EnemyComboBonus);
			AddToScore(num4, notify);
			PlayerStats.IncreaseLongStat(PlayerStats.LongStatNames.Score_BoosterEnemyCombos_Total, num4);
			if (CharacterManager.Singleton.GetCurrentCharacter() == Characters.Type.Espio)
			{
				PlayerStats.IncreaseLongStat(PlayerStats.LongStatNames.Score_BoosterEnemyCombosAsEspio_Total, num4);
			}
		}
		if (enemy.Golden)
		{
			int additionalScore = (int)((float)(num3 * GetMultiplier(ScoreCategory.Enemy)) * Boosters.GoldenEnemyScoreMultipler);
	...
	private void Event_OnRingStreakCompleted(int length, float firstRingTrackPosition, float lastRingTrackPosition)
	{
		if (Boosters.IsBoosterSelected(PowerUps.Type.Booster_RingStreakBonus))
		{
			...
			int num = (int)(Boosters.RingStreakBonusScore * GetMultiplier(ScoreCategory.Streak));
			AddToScore(num, ScoreNotify.RingStreakBonus);
			EventDispatch.GenerateEvent("OnBoosterActivated", PowerUps.Type.Booster_RingStreakBonus);
			PlayerStats.IncreaseLongStat(PlayerStats.LongStatNames.Score_BoosterRingStreaks_Total, num);
		}
	}
	...
	private void Event_OnSpringStart(SpringTV.Type springType)
	{
		...
		{
			PlayerStats.IncreaseStat(PlayerStats.StatNames.BoosterSpringSprings_Total, 1);
			if (CharacterManager.Singleton.GetCurrentCharacter() == Characters.Type.Rouge)
			{
				PlayerStats.IncreaseStat(PlayerStats.StatNames.SpringsBoosterBonusesAsRouge_Total, 1);
			}
			int additionalScore = (int)(PlayerStats.GetCurrentStats().m_trackedStats[91] * Boosters.SpringBonusScore * GetMultiplier(ScoreCategory.Distance));
