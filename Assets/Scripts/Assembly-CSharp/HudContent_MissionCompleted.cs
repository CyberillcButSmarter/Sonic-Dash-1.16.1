...
public class HudContent_MissionCompleted
{
	...
	public HudContent_MissionCompleted(GameObject displayRoot, GameObject displayTrigger, UILabel description, float displayDuration, AudioClip missionCompletedAudio)
	{
		m_mission = description;
	...
	private void Display()
	{
		if (m_mission != null)
		{
			int missionGroup = m_notifications[m_currentNotification].m_missionGroup;
			MissionTracker.GetMissionDescription(missionGroup, m_mission);
		}
		m_displayTrigger.SendMessage("OnClick", SendMessageOptions.RequireReceiver);
		Audio.PlayClip(m_missionCompletedAudio, false);
