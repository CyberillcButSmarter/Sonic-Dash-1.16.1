...
public class TrackSegment : MonoBehaviour
{
	public class LaneCollection : IEnumerable, IEnumerable<int>
	{
		...
		public LaneCollection(IEnumerable<JoinPoint> joinPoints, float laneWidth)
		{
			...
			foreach (JoinPoint join in joinPoints)
			{
				...
				Func<int, Vector3> func = delegate(int laneIndex)
				{
					switch (laneCount)
					{
					case 1:
						return join.Transform.position;
					case 2:
						return join.Transform.position + (laneIndex - join.Lanes.First()) * join.Transform.right * (0f - laneWidth);
					case 3:
						return join.Transform.position + (laneIndex - join.Lanes.ElementAt(1)) * join.Transform.right * (0f - laneWidth);
					default:
						throw new UnityException("can't handle track joins with more than three pieces!");
					}
				};
	...
	public struct CameraKeyframe
	{
		...
		public bool IsValid
		{
			get
			{
				return m_cameraTransform != null;
			}
		}

		public string CameraName { get; private set; }

		public LightweightTransform CameraTransform
		{
			get
			{
				return new LightweightTransform(m_cameraTransform.position, Quaternion.LookRotation(-m_cameraTransform.forward, m_cameraTransform.up));
			}
		}
	...
	public IEnumerable<Transform> TemplateContainers
	{
		get
		{
			return m_templateContainers;
		}
	}

	public IEnumerable<GameplayTemplate> TemplatesStartedOnThis
	{
		get
		{
			return m_templatesStartedOnThisSegment;
		}
	}
	...
	public bool IsMissingAnyLanes
	{
		get
		{
			return TrackDatabase.IsGapType(Template.PieceType.Type);
		}
	}

	public LaneCollection EntranceLanes
	{
		get
		{
			return m_entranceLanes;
		}
	}

	public LaneCollection ExitLanes
	{
		get
		{
			return m_exitLanes;
	...
	public IEnumerable<JoinPoint> EntrancePoints
	{
		get
		{
			return m_entrances;
	...
	public IEnumerable<JoinPoint> ExitPoints
	{
		get
		{
			return m_exits;
	...
	public IEnumerable<CameraKeyframe> SetpieceCameraKeyframes
	{
		get
		{
			return m_setpieceCameraKeyframes;
		}
	}

	public int SetPieceStartCameraTransitionKeyFrame
	{
		get
		{
			return m_setPieceStartCameraTransitionKeyFrame;
		}
	}

	public bool IsSetpieceCameraAvailable
	{
		get
		{
			return SetpieceCameraKeyframes.Any();
		}
	}
