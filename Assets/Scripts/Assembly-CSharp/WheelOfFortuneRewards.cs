...
public class WheelOfFortuneRewards : MonoBehaviour
{
	...
	public bool[] ValidJackpots
	{
		get
		{
			return m_validJackpots;
		}
	}
	...
	{
		for (int i = 0; i < Instance.m_normalRewardIDs.Length; i++)
		{
			if (string.Compare(Instance.m_normalRewardIDs[i], rewardToReplace, StringComparison.InvariantCultureIgnoreCase) == 0)
			{
				Instance.m_normalRewardIDs[i] = newReward;
			}
		}
		for (int j = 0; j < Instance.m_jackpotRewardIDs.Length; j++)
		{
			if (string.Compare(Instance.m_jackpotRewardIDs[j], rewardToReplace, StringComparison.InvariantCultureIgnoreCase) == 0)
			{
				Instance.m_jackpotRewardIDs[j] = newReward;
			}
		}
	}
