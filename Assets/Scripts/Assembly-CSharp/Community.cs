...
public class Community : MonoBehaviour
{
	...
	private bool m_FBAuthenticating;

	public LeaderboardPopulator LeaderboardPopulator
	{
		get
		{
			return m_leaderboardPopulator;
		}
	}
	...
	private void Start()
	{
		...
		EventDispatch.RegisterInterest("OnAppLeavesBackground", this);
	...
	public void InviteFBFriend(string user)
	{
		Trigger_FBInviteFriend(user);
	}
	...
	public void Event_OnAppLeavesBackground()
	{
		StartCoroutine(TestFacebookLoginDismissed());
	}

	private IEnumerator TestFacebookLoginDismissed()
	{
		yield return new WaitForSeconds(1f);
		if (m_FBAuthenticating)
		{
			OnFacebookAuthenticateComplete("false");
		}
	}
	...
	private void Trigger_FBLogin()
	{
		...
		{
			EventDispatch.GenerateEvent("OnGrantooFacebookAuthenticateComplete", new bool[1] { true });
			return;
		}
		PropertyStore.Save();
		if (m_socialPlatform != null)
		{
			m_FBAuthenticating = true;
	...
	private void Trigger_FBInviteFriend(string user)
	{
		if (!((HLLocalUser)Social.localUser).isFacebookAuthenticated)
		{
			m_waitingForToInvite = true;
		...
		{
			((HLSocialPlatform)Social.Active).InviteFriend(user);
		}
	}

	private void Trigger_ShareVideo()
	{
		EventDispatch.GenerateEvent("ShareVideoRequest");
	}
	...
	private void Event_MainMenuActive()
	{
		...
		int num = currentStats.m_trackedStats[80];
		if (currentStats.m_trackedStats[61] == 0 && !isFacebookAuthenticated && (m_fbPopupShowedTimes == 0 || (m_fbPopupShowedTimes <= 5 && num >= m_fbPopupLasSessionShown + 3) || (m_fbPopupShowedTimes <= 10 && num >= m_fbPopupLasSessionShown + 5)))
		{
