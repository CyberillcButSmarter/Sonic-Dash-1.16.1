...
public class WaterAlpha : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		Vector2 textureScale = m_targetPlane.GetComponent<Renderer>().material.GetTextureScale("_MainTex");
	...
	private void Update()
	{
		if ((bool)m_targetPlane)
		{
			...
			scrollValueA.x %= 1f;
			scrollValueA.y %= 1f;
			m_targetPlane.GetComponent<Renderer>().material.SetTextureOffset("_MainTex", scrollValueA);
			...
			scrollValueA.x %= 1f;
			scrollValueA.y %= 1f;
			m_targetPlane.GetComponent<Renderer>().material.SetTextureOffset("_SecondTex", scrollValueA);
		}
	}
