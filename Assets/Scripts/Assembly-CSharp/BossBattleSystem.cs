...
public class BossBattleSystem : MonoBehaviour
{
	...
	public class Phase
	{
		public enum Types
		{
			Arrive = 0,
			Intro = 1,
			TransitionToAttack1 = 2,
			Attack1Intro = 3,
			Attack1 = 4,
			TransitionToAttack2 = 5,
			Attack2 = 6,
			TransitionToVulnerable = 7,
			Vulnerable = 8,
			Leave = 9,
			Finish = 10
		}
		...
		public GestureSettings CurrentGesture
		{
			get
			{
	...
	public int DefaultBossScore
	{
		get
		{
			return m_defaultBossScore;
		}
	}
	...
	{
		int num = PlayerStats.GetCurrentStats().m_trackedStats[91];
	...
	public int GetNextPhaseIndex(int phaseIndex)
	{
		if (phaseIndex < m_phases.Count)
		{
			...
			if (phaseIndex < m_phases.Count && (m_phases[phaseIndex].Type == Phase.Types.Attack1 || m_phases[phaseIndex].Type == Phase.Types.Attack2))
			{
				...
				PlayerStats.StatNames statNames = ((m_difficulty != DifficultySetting.Easy) ? PlayerStats.StatNames.BossBattlesHard_Total : PlayerStats.StatNames.BossBattlesEasy_Total);
	...
	{
		GameplayTemplate.Group gToFind = GameplayTemplate.Group.BossBattle;
		...
		while (flag)
		{
			...
			if (!current.ContainsGroup(gToFind))
			{
				continue;
			}
