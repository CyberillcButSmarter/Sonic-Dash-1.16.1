...
public class CameraTypeSetPiece : CameraType
{
	...
	public void ActivateSetPiece(Transform entryTransform, IEnumerable<TrackSegment.CameraKeyframe> cameraKeyframes, int startTranstionFrame)
	{
		...
		for (int num = 0; num < cameraKeyframes.Count(); num++)
		{
			TrackSegment.CameraKeyframe cameraKeyframe = cameraKeyframes.ElementAt(num);
			array[num] = new LightweightTransform(entryTransform.InverseTransformPoint(cameraKeyframe.CameraTransform.Location), cameraKeyframe.CameraTransform.Orientation);
		}
		m_cameraSpline = new SubSpline[cameraKeyframes.Count()];
		for (int num2 = -1; num2 < cameraKeyframes.Count() - 1; num2++)
		{
			m_cameraSpline[num2 + 1] = new SubSpline(num2, m_startCameraLocalTransform, array, m_splineStepping, m_splineResolution);
		}
