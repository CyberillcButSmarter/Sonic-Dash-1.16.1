...
public class GameMusic : MonoBehaviour
{
	...
	public AudioClip m_bossZazzAudioClip;

	public AudioClip m_bossEggmanAudioClip;
	...
	public static GameMusic Singleton
	{
		get
		{
			return s_singleton;
	...
	private void Update()
	{
		...
		{
			m_music.Play(m_gameAudioClip, true, m_resultsVolume);
	...
	private void Event_StartGameState(GameState.Mode state)
	{
		...
		case GameState.Mode.Menu:
			m_music.Play(m_menuAudioClip, true, m_fullVolume);
		...
		case GameState.Mode.Game:
			if (m_bossBattleMusicStarted)
			{
				...
				{
					m_music.Play(m_bossZazzAudioClip, true, m_fullVolume);
				}
				else if (BossLoader.Instance().m_currentBoss == BossLoader.Bosses.Eggman)
				{
					m_music.Play(m_bossEggmanAudioClip, true, m_fullVolume);
				}
			}
			else
			{
				m_music.Play(m_gameAudioClip, true, m_fullVolume);
			}
			m_state = State.Game;
			break;
	...
	{
		m_music.Play(m_gameAudioClip, true, m_fullVolume, true);
	...
	private void Event_BossMusicStart()
	{
		if (!m_bossBattleMusicStarted)
		{
			...
			if (BossLoader.Instance().m_currentBoss == BossLoader.Bosses.Zazz)
			{
				m_music.Play(m_bossZazzAudioClip, true, m_fullVolume, true);
			}
			else if (BossLoader.Instance().m_currentBoss == BossLoader.Bosses.Eggman)
			{
				m_music.Play(m_bossEggmanAudioClip, true, m_fullVolume, true);
			}
		}
	}
	...
	private IEnumerator GameMusicFadeUp(float fadeTime)
	{
		...
		m_music.Play(m_gameAudioClip, true, 0f, true);
		float timer = 0f;
