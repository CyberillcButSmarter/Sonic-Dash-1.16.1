...
public class Dialog_TargetedOffer : MonoBehaviour
{
	...
	private void UpdateContent()
	{
		...
		string text3 = LanguageStrings.First.GetString(text);
		string format = LanguageStrings.First.GetString(text + "_BODY");
		string arg = LanguageStrings.First.GetString(text2);
		m_costBefore.text = string.Empty;
		m_costNow.text = string.Empty;
		string text4 = string.Format(format, s_offer.m_percentOff, arg);
		m_title.text = text3;
		...
		format = LanguageStrings.First.GetString("TARGETED_OFFERS_RED_STAR_RINGS");
		m_itemOffer.text = string.Format(format, storeEntry2.m_awards.m_playerStars);
		format = LanguageStrings.First.GetString("TARGETED_OFFERS_OFF");
		m_itemOfferDiscount.text = string.Format(format, s_offer.m_percentOff.ToString());
		...
		{
			string playerPrice = storeEntry3.m_osStore.m_playerPrice;
			string format2 = LanguageStrings.First.GetString("STORE_SALE_PRICE_WAS");
			m_costBefore.text = string.Format(format2, playerPrice);
		}
		...
		{
			string playerPrice2 = storeEntry4.m_osStore.m_playerPrice;
			string format3 = LanguageStrings.First.GetString("STORE_SALE_PRICE_NOW");
			m_costNow.text = string.Format(format3, playerPrice2);
		}
	}
